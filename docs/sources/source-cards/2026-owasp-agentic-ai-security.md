...
- 适合阶段：工程实践 / 生产化
- 可信度等级：A/B
- 是否已验证：公开资源页、HTTP metadata、WordPress JSON 摘要和附件列表 2026-07-12 已复核；可支撑 agentic AI 风险分类和治理 gap 的保守边界；`Agentic AI - Threats and Mitigations` 下载端点仍重定向到 no-access，页面附件列表只公开封面图片，不能作为可读白皮书全文证据；真实缓解效果仍未验证
...
- 2026-07-12 复核：`Agentic AI - Threats and Mitigations` 页面返回 HTTP 200；HEAD metadata 显示页面层 `last-modified: Tue, 07 Jul 2026 00:05:34 GMT`；WP JSON resource id `45677` 字段仍显示 `date: 2025-02-17T11:19:52`、`modified: 2025-04-28T11:35:15`。因此只把公开摘要当作已复核证据，不把页面层 header 当作白皮书正文已更新或已精读的证据。
...
- 2026-07-12 复核：`AIUC-1 Crosswalks OWASP Top 10 For Agentic Applications` 页面返回 HTTP 200；WP JSON resource id `54630` 显示 `date: 2026-05-25T09:37:49`、`modified: 2026-05-25T09:37:49`。
...
- 2026-07-12 复核：`State of Agentic AI Security and Governance 2.01` 页面返回 HTTP 200；HEAD 链接到 WP JSON resource id `50595`；WP JSON 显示 `date: 2026-06-01T22:48:51`、`modified: 2026-06-08T12:35:04`，摘要说明该报告面向 autonomous AI systems 的安全、治理、frameworks、governance models 和 regulatory standards。
- `Agentic AI - Threats and Mitigations` 下载端点 `https://genai.owasp.org/download/45674/` 当前返回 HTTP 302 并重定向到 `https://genai.owasp.org/no-access/download-id/45674/`；猜测的 PDF 直链 `https://genai.owasp.org/wp-content/uploads/2025/02/Agentic-AI-Threats-and-Mitigations_v1.0.pdf` 返回 HTTP 404；`/wp-json/wp/v2/media?parent=45677` 只返回封面 PNG 附件 `Agentic-AI-Threats-and-Mitigations_v1.0-Cover.png`。因此本卡没有把白皮书全文内容当成已精读证据。

## 可能的问题

- 当前可验证内容主要来自公开资源页、WP JSON 摘要和封面/附件 metadata，不是完整白皮书正文。

...
- 作者 / 机构：OpenAI
- 发布时间：持续更新示例；Cookbook 首页 last-modified 复核为 2026-07-11；Usage/Cost recipe 发布时间为 2025-01-13
- 最后复核日期：2026-07-12
- 类型：Official Examples / Engineering
- 主题：LLM Application Development / Tool Use / RAG / Evals
- 适合阶段：入门 / 工程实践
- 可信度等级：A/B
- 是否已验证：索引页与若干具体 recipe 已于 2026-07-12 复核 HTTP metadata；支撑“Cookbook 具体 recipe 可作为实践项目参考，但不能替代 API 文档、生产安全指南或本地实验”的窄边界可入正文；Usage/Cost 和 Rate limits recipe 可支撑练习字段设计；实践路线 smoke harness 已补 project readiness audit，验证跟练项目需要 prerequisites、commands、acceptance checks、trace fields、failure examples、references 和 boundaries；真实试跑体验、账户数据和 API 行为仍部分验证
...
- `https://cookbook.openai.com/` 重定向到 `https://developers.openai.com/cookbook`；`https://developers.openai.com/cookbook` 于 2026-07-12 HEAD 复核返回 HTTP 200，`last-modified: Sat, 11 Jul 2026 06:08:52 GMT`，`content-type: text/html; charset=utf-8`。
- 已复核 recipe 页面：
  - `https://developers.openai.com/cookbook/examples/structured_outputs_intro`：2026-07-12 返回 HTTP 200，`last-modified: Sat, 11 Jul 2026 07:35:40 GMT`。
  - `https://developers.openai.com/cookbook/examples/file_search_responses`：2026-07-12 返回 HTTP 200，`last-modified: Sat, 11 Jul 2026 06:36:19 GMT`。
  - `https://developers.openai.com/cookbook/examples/evaluation/getting_started_with_openai_evals`：2026-07-12 返回 HTTP 200，`last-modified: Sat, 11 Jul 2026 07:47:18 GMT`。
  - `https://developers.openai.com/cookbook/examples/agents_sdk/evaluate_agents`：2026-07-12 返回 HTTP 200，`last-modified: Sat, 11 Jul 2026 12:05:54 GMT`。
  - `https://developers.openai.com/cookbook/examples/completions_usage_api`：2026-07-12 返回 HTTP 200，`last-modified: Sat, 11 Jul 2026 06:19:49 GMT`。
  - `https://developers.openai.com/cookbook/examples/how_to_handle_rate_limits`：2026-07-12 返回 HTTP 200，`last-modified: Sat, 11 Jul 2026 19:02:32 GMT`。

### Usage / Cost recipe 复核

- URL：`https://developers.openai.com/cookbook/examples/completions_usage_api`
- HTTP metadata：2026-07-12 复核返回 HTTP 200；canonical URL 为上述页面；content-type 为 `text/html; charset=utf-8`；last-modified 为 `Sat, 11 Jul 2026 06:19:49 GMT`。
...
- HTTP metadata：2026-07-12 复核返回 HTTP 200；canonical URL 为上述页面；content-type 为 `text/html; charset=utf-8`；last-modified 为 `Sat, 11 Jul 2026 19:02:32 GMT`。
...
- 边界：Tenacity/backoff 属于第三方工具，页面示例不能保证其可靠性或安全性；本 repo 没有真实触发 rate limit，也没有验证 retry、主动节流、batching、fallback 或并发脚本在当前账户和 workload 下的效果；页面的示例输出和 notebook 结果不应写成本地已复现。

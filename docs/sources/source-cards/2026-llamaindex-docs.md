...
- 适合阶段：入门后 / 工程实践
- 可信度等级：A
- 是否已验证：入口、RAG 总览和核心组件页面已复核；RAG 工程 pipeline 窄边界已可入正文；跨框架术语对照第一轮已完成；标准库最小 pipeline / citation 模拟实验和上下文策略对比实验已完成；本地 LlamaIndex `VectorStoreIndex` / retriever / source-node metadata run 已完成
...
- 2026-07-11 运行 Real LlamaIndex RAG Source-Node Validation：使用 `llama-index-core`、`VectorStoreIndex` 和本地 deterministic keyword embedding 跑通 4 个 `Document`、3 个 query case、retriever source-node metadata、应用层 trust filter、chunk-level citation metadata 和无证据 `grounded=false`；结果 `all_passed=true`、`untrusted_chunk_cited=false`、`secret_leaked_in_trace=false`。
...
- LlamaIndex docs 本身主要支撑工程流程和组件边界；`source_nodes` / citation 的代码形态已由 LlamaIndex examples repo source card 抽样补强。本地 `VectorStoreIndex` run 只支撑真实框架 retrieval/source-node metadata 的最小观察；真实 LlamaIndex / LLM synthesis 下的 citation correctness、faithfulness、embedding / vector DB / reranker 表现仍需后续实验。
...
- 已准备真实 LLM citation synthesis harness：本地关键词检索提供 chunk，上游 LLM 生成结构化 answer / grounded / citations，并校验 citation id 和最小语义；当前无 API key，已记录 skipped 结果，不能提前升级真实 citation correctness 结论。
- 已完成 Real LlamaIndex RAG Source-Node Validation：本地 `llama-index-core` / `VectorStoreIndex` / retriever run 验证 source-node metadata、trust filter、citation metadata 和无证据 `grounded=false` 的最小结构；没有运行 LLM synthesis。
...
- 原因：RAG、Memory 和框架生态章节需要现代工程 reference。当前已可支撑 RAG 的 loading、indexing、storing、querying/retrieval、response synthesis、evaluation、chunk metadata、retrieval trace、citation/source 绑定、retriever/query engine 术语边界，以及本地 LlamaIndex retrieval/source-node metadata 的最小观察；真实 LLM citation correctness、answer faithfulness、embedding / vector store、chunk size/top-k/rerank、latency、token cost 和生产权限边界仍需实验。

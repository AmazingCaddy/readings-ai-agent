...
- 适合阶段：入门后 / 工程实践
- 可信度等级：A
- 是否已验证：入口、RAG 总览和核心组件页面已复核；RAG 工程 pipeline 窄边界已可入正文；跨框架术语对照第一轮已完成；标准库最小 pipeline / citation / retrieval strategy audit 和上下文策略对比实验已完成；本地 LlamaIndex `VectorStoreIndex` / retriever / QueryEngine / `MockLLM` / source-node metadata run 已完成

...
- 适合阶段：工程实践 / 生产化前检查
- 可信度等级：A
- 是否已验证：官方 Markdown 页面 2026-07-12 返回 HTTP 200；关键段落已复核；可支撑 moderation 作为检测信号、输入/输出分类、类别/分数解释、tool-calling 覆盖边界、streaming 限制和 `/v1/moderations` 数据保留边界；标准库 production safety / data governance checklist + object-level data-flow audit 已完成；Real Moderation Safety harness 已完成本地 deterministic policy-signal control，当前无 API key 未验证真实 API；真实误报、漏报、策略阈值、人工复核和生产效果仍部分验证
...
- Data controls 文档中 `/v1/moderations` 行显示 abuse monitoring retention 为 `None`、application state retention 为 `None`、Zero Data Retention eligible 为 `Yes`；这只支撑该 endpoint 的数据保留边界，不代表工具调用、Responses、文件、vector stores、hosted containers 或第三方工具没有保留数据。
- 部分类别只支持 text，部分支持 text and images；图片文件大小有限制。

## 支撑证据

- 2026-07-12 复核：`moderation.md` 返回 HTTP 200；`last-modified: Sat, 11 Jul 2026 17:54:18 GMT`；`content-type: text/markdown; charset=utf-8`。
- 2026-07-12 复核：`your-data.md` 的 endpoint retention matrix 中 `/v1/moderations` 行显示 data used for training 为 `No`、abuse monitoring retention 为 `None`、application state retention 为 `None`、Zero Data Retention eligible 为 `Yes`。
...
- 已完成标准库 production safety / data governance checklist + object-level data-flow audit，验证 moderation policy signal、tool-calling 覆盖、streaming 时机以及相关安全数据流可以拆成可审计字段。
- 已准备 [Real Moderation Safety Validation](../../experiments/real-moderation-safety-validation/README.md) harness：无 API key 时运行本地 deterministic policy-signal control，并标记 `api_status=skipped_without_openai_api_key`、`real_api_validated=false`；本次覆盖 `flagged`、categories、category scores、`category_applied_input_types`、expected mismatch、true positive、true negative、false positive、false negative 和 policy decision 分支。有 API key 时可记录真实 `flagged`、categories、category scores、latency、expected mismatch、tool arguments / tool output 样本和 policy decision。当前尚未产生真实 moderation API completed run。
...
- 原因：官方文档直接支撑 moderation 的输入/输出、结果字段、tool-calling 覆盖边界和 streaming 限制；标准库 checklist 和本地 policy-signal control 支撑记录模板和应用层分支检查。仍需保守写明：它不证明具体阈值、策略、检测层或生产安全效果充分可靠。

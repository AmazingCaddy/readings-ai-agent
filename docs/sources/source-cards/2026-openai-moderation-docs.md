...
- 适合阶段：工程实践 / 生产化前检查
- 可信度等级：A
- 是否已验证：官方 Markdown 页面返回 HTTP 200；关键段落已精读；可支撑 moderation 作为检测信号、输入/输出分类、类别/分数解释、tool-calling 覆盖边界和 streaming 限制的工程边界；标准库 production safety / data governance checklist audit 已完成；Real Moderation Safety harness 已完成本地 deterministic policy-signal control，当前无 API key 未验证真实 API；真实误报、漏报、策略阈值、人工复核和生产效果仍部分验证
...
- 已完成标准库 production safety / data governance checklist audit，验证 moderation policy signal、tool-calling 覆盖和 streaming 时机可以拆成可审计字段。

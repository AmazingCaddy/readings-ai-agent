...
- 可信度等级：B+
- 是否已验证：官方 docs index、Guided Tour、Intro Agents、Secure Code Execution、Tools reference、GitHub repo metadata 和 README 已于 2026-07-12 复核；Real smolagents validation 已用 `smolagents==1.26.0`、deterministic fake models 和本地 tools 跑通 `CodeAgent` / `ToolCallingAgent` runtime surface、`output_schema` 非强制校验和默认 local executor import allowlist 表面；可支撑 smolagents 的框架定位、CodeAgent / ToolCallingAgent 范式、工具 metadata、Hub/MCP 工具信任边界和代码执行边界；没有真实模型、MCP server、Hub tool 或远程 sandbox 运行，不能验证框架质量、benchmark claim、真实成本、延迟、安全隔离效果或生产可靠性
...
- Real smolagents validation 已用 `smolagents==1.26.0` 和 deterministic fake models 观察到：`CodeAgent` 可执行 Python snippet 调本地 tools，`ToolCallingAgent` 可执行 structured tool calls，声明了 `output_schema` 的 tool 返回 plain string 时 runtime 没有自动做 output validation，默认 local executor 对 `import os` 返回 not allowed，trace 未泄露 untrusted fixture 中的示例 secret marker。该实验只支撑本地 runtime surface，不支撑真实模型、远程 sandbox、Hub/MCP tools 或生产质量。
...
- [Real smolagents validation](../../experiments/real-smolagents-validation/README.md) 于 2026-07-12 用 `uv run --with smolagents` 安装临时依赖并运行 `smolagents==1.26.0`；结果见 [2026-07-12 results](../../experiments/real-smolagents-validation/results-2026-07-12.md)。输出摘要记录 `code_agent_validated=true`、`tool_calling_agent_validated=true`、`output_schema_not_enforced_observed=true`、`local_python_executor_import_block_observed=true`、`fake_model_only=true`、`real_model_validated=false`、`real_api_validated=false`。
...
- 本卡已经完成 `smolagents==1.26.0` 的本地 fake-model runtime surface，但没有使用真实模型、MCP server、Hub tool、Docker/E2B/Blaxel/Modal sandbox，也没有测真实成本、延迟、trace 平台质量或安全隔离效果。
...
- 已完成同一只读 mock tool 的 `CodeAgent` / `ToolCallingAgent` deterministic fake-model 对照；后续应扩展到真实模型、真实错误回传、LOC、trace 可读性、成本和失败样例。
...
- 结论：部分进入
- 原因：可进入第 10 章作为 code action vs structured tool calling 的框架样本，并进入第 11 章作为 Hugging Face Agents Course 之后的可选实践路线补充。可进入的只有框架定位、agent 类型、tool metadata、Hub/MCP trust boundary、local executor 不是安全边界、`output_schema` 非强制校验、本地 fake-model runtime surface、sandbox 方案取舍等窄结论；不能用于证明 smolagents 真实模型效果、benchmark 排名、框架质量、成本、延迟、安全隔离或生产可靠性。

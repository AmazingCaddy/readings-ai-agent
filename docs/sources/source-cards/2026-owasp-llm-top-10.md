...
- 已准备真实 Responses API prompt injection / permission harness：当前无 API key，已有 skipped 结果页；配置后可记录模型是否请求高风险写工具、prompt-only 模拟副作用、policy-enforced 模拟人工拒绝和 trace 脱敏结果。真实 completed run 仍待跑，不能提前升级结论。

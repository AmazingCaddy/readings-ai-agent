...
- 适合阶段：入门后 / 进阶
- 可信度等级：A
- 是否已验证：来源链接、arXiv 元数据和摘要已复核；与 LangGraph memory docs 完成边界交叉验证；RAG / Memory 术语边界已可入正文
...
- 因此本手册可把 RAG 作为“外部知识检索增强生成”的基础机制，但需要提醒初学者：RAG 论文中的 non-parametric memory 术语不等同于 Agent 长期记忆治理。
...
- 结论：进入；术语边界可入正文
- 原因：RAG 章节基础 reference；可支撑 RAG 的外部检索、provenance、知识更新动机，以及 non-parametric memory 不等同于 Agent long-term memory 的边界。真实工程 RAG 质量仍需现代框架和实验补充。

...
- 适合阶段：入门 / 工程实践
- 可信度等级：A
- 是否已验证：Structured Outputs 页面已复核；“schema valid 不等于事实/权限/业务正确”边界已可入正文；上下文治理与结构化输出标准库实验已完成；Real Structured Outputs / JSON Mode harness 已准备并接入统一 runner，当前无 API key 只验证 skip 分支
...
- 已准备 [Real Structured Outputs / JSON Mode 对比实验](../../experiments/real-structured-output-validation/README.md) harness：无 API key 时返回 `skipped`；配置后可记录 free text、JSON mode、json_schema 的 schema valid / semantic valid / refusal 行为。当前结果页只记录 skip 分支，尚未产生真实 completed run，不能提前升级结论。

## 是否进入正文

- 结论：进入；窄边界可入正文
- 原因：第 02/03 章需要官方文档支撑结构化输出、JSON mode 和 function calling 的边界；官方文档和标准库实验共同支撑“schema adherence 不等于事实/业务正确”；真实 harness 准备工作支撑后续记录模板。真实 Responses API refusal、retry、跨模型稳定性和成本仍需实测。

...
- 适合阶段：入门 / 工程实践
- 可信度等级：A
- 是否已验证：Structured Outputs 页面已复核；“schema valid 不等于事实/权限/业务正确”边界已可入正文；上下文治理与结构化输出标准库实验已完成
...
- 已准备真实 Responses API Structured Outputs / JSON mode harness：无 API key 时跳过，配置后可记录 free text、JSON mode、json_schema 的 schema valid / semantic valid / refusal 行为；结果待跑，不能提前升级结论。

## 是否进入正文

- 结论：进入；窄边界可入正文
- 原因：第 02/03 章需要官方文档支撑结构化输出、JSON mode 和 function calling 的边界；官方文档和标准库实验共同支撑“schema adherence 不等于事实/业务正确”。真实 Responses API refusal、retry、跨模型稳定性和成本仍需实测。

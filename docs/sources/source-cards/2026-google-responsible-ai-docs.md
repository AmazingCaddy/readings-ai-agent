...
- 发布时间：文档持续更新；HTTP `last-modified` 为 2026-07-08
- 最后复核日期：2026-07-11
...
- 可信度等级：A
- 是否已验证：HTTP 200、canonical URL、页面标题和关键正文段落已复核；支撑模型限制、grounding/factuality、数据质量、输入输出长度/结构限制、安全过滤、按用例安全测试、用户反馈和内容监控的保守边界；不证明任意安全过滤、grounding、监控或 Google 模型在具体业务中的真实效果
...
- 2026-07-11 使用 `curl -L -I` 复核旧 Vertex AI responsible AI URL，最终跳转到 `https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/responsible-ai` 并返回 HTTP 200。
- 2026-07-11 使用 `curl -L -I` 复核最终 URL，HTTP header 包含 `last-modified: Wed, 08 Jul 2026 22:44:04 GMT`。
- 2026-07-11 抓取页面正文成功；页面标题为 `Responsible AI | Gemini Enterprise Agent Platform | Google Cloud Documentation`。

...
- 适合阶段：进阶 / 工程实践
- 可信度等级：A
- 是否已验证：overview、Agent Framework、Plugins 和 Process Framework 页面已复核；框架生态边界和跨框架术语对照已完成第一轮交叉验证；高风险工具权限窄边界可入正文；审批状态恢复实验已完成标准库模拟；Semantic Kernel native plugin runtime 窄观察已完成；真实模型 / OpenAPI / MCP plugin / HITL 行为仍部分验证
...
- 本地 native plugin run 不覆盖真实模型 tool selection、OpenAPI plugin、MCP plugin、Agent Framework、Process Framework 或 HITL UI；高风险写操作审批仍由应用层 wrapper 实现，不能写成框架默认自动保证。

...
- `docs/examples/index.md` 把 examples 分成 Agents、Agentic Workflows、LLM Integrations、Embedding Models、Vector Stores 等入口，适合补充实践路线的广度。
...
- GitHub API 元数据显示仓库 `run-llama/llama_index` 为 public，MIT license，默认分支 `main`，语言为 Python，topics 包含 `agents`、`framework`、`rag`、`vector-database`，创建时间为 2022-11-02。
...
- `docs/examples/index.md` 说明 examples 是 rich collection of examples，并列出 Function Calling Agent、ReAct Agent、Multi-Agent Workflow、Basic RAG、Advanced Text-to-SQL、OpenAI/Anthropic/Ollama integrations、OpenAI/HuggingFace/Ollama embeddings、Pinecone/Chroma/Qdrant/Azure AI Search vector stores 等入口。
...
- 抽样复核 `citation_query_engine.ipynb`：notebook 导入 `CitationQueryEngine`，通过 `CitationQueryEngine.from_args(index, similarity_top_k=3, citation_chunk_size=512)` 创建 query engine，并打印 `len(response.source_nodes)` 与 source node 文本。
- 抽样复核 `workflow/rag.ipynb`：notebook 标题为 `RAG Workflow with Reranking`，说明 RAG + reranking 由 indexing、retrieval、rerank、synthesizing final response 等步骤组成，并定义 `RetrieverEvent`、`RerankEvent` 与 workflow steps。
- 抽样复核 `retrievers/bm25_retriever.ipynb`：notebook 使用 `SentenceSplitter(chunk_size=512)` 生成 nodes，使用 `BM25Retriever`、docstore persistence、metadata filter、Chroma vector store 和 `QueryFusionRetriever` 展示 sparse/dense hybrid retrieval。
...
- examples 会随仓库版本变化，notebook 依赖、API 参数和默认模型可能失效，需要记录复核日期。

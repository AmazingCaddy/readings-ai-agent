...
- 适合阶段：入门后 / 架构历史
- 可信度等级：A
- 是否已验证：来源链接、HTTP metadata、arXiv 元数据和摘要已复核；支撑“把 LLM 与外部知识、离散推理模块组合成系统架构”的窄边界；真实工具路由质量、现代 function calling 行为、成本、延迟和框架表现仍部分验证
...
- arXiv 页面返回 HTTP 200；HTTP `last-modified` 为 2022-05-03。
- arXiv 元数据显示 submitted on 2022-05-01，当前版本 v1。
- 摘要写明 LMs are inherently limited in a number of ways，并讨论 adopting a systems approach。
- 摘要写明 MRKL conceptualizes the challenge as one that involves knowledge and reasoning in addition to linguistic processing。
- 摘要写明 architecture uses multiple neural models, complemented by discrete knowledge and reasoning modules。

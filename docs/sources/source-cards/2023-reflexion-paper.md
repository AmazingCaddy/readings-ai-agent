...
- 适合阶段：进阶
- 可信度等级：A
- 是否已验证：来源链接、arXiv 元数据和摘要已复核；复杂架构不是默认更可靠的窄边界可入正文；Reflection 效果仍部分验证
...
- 已与 ReAct、Tree of Thoughts、LangGraph、长期记忆治理 evidence、eval evidence 和 reflection/retry 标准库实验交叉验证 reflection 边界；可支撑“复杂架构不是默认更可靠”的窄结论，但不能证明 reflection 在真实任务中稳定提升。

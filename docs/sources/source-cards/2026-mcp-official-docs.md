...
- 适合阶段：入门 / 工程实践
- 可信度等级：A
- 是否已验证：来源链接已复核；architecture、server concepts、client concepts、2025-11-25 spec tools/resources/prompts/authorization/roots/sampling/elicitation 和 security best practices 关键段落已精读；host/client/server 职责边界和安全/授权窄边界已可入正文；MCP 最小 trace 标准库模拟和 stdio harness 已完成；真实 host 行为仍部分验证

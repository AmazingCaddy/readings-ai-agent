...
- 适合阶段：入门 / 工程实践
- 可信度等级：A
- 是否已验证：来源链接已复核；architecture、server concepts、client concepts、2025-11-25 spec tools/resources/prompts/authorization/roots/sampling/elicitation 和 security best practices 关键段落已精读；核心职责、安全与权限边界已交叉验证
...
- Security Best Practices 是 implementation guidance；它能支撑风险和缓解方向，但仍需要本地实验验证具体 host/client/server 的行为。

## 是否进入正文

- 结论：进入
- 原因：MCP 章节必须优先引用官方文档；安全与权限相关结论应标注为“部分验证”，直到完成最小 MCP trace 和本地权限实验。

...
- 已准备真实 Batch / Flex / Prompt Caching harness：`docs/experiments/real-batch-flex-caching-validation/real_batch_flex_caching_validation.py`。无 `OPENAI_API_KEY` 时运行本地 deterministic metadata control，并标记 `real_api_validated=false`；配置 API key 后可记录 Prompt Caching usage 字段、Flex response / API error 和 Batch JSONL / `custom_id` metadata。Batch job 提交默认 opt-in。

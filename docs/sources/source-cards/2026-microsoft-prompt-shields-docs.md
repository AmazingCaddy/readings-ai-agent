...
- 可信度等级：A
- 是否已验证：Microsoft Learn HTTP 200、页面元数据、Prompt Shields concept page 和 quickstart 关键段落已复核；支撑 user prompt attack、document attack、`shieldPrompt` API、`attackDetected` 返回字段和 false positive / false negative 边界；不证明真实攻击拦截率、误报率、漏报率、跨模型效果或生产安全充分性
...
- 2026-07-11 使用 `curl -L -I` 复核 concept page，返回 HTTP 200；页面标题为 `Prompt Shields in Azure AI Content Safety - Azure AI services`。
...
- 2026-07-11 使用 `curl -L -I` 复核 quickstart，返回 HTTP 200；页面标题为 `Quickstart: Detect prompt attacks with Prompt Shields - Azure AI services`。

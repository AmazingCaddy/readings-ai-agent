...
- 适合阶段：工程实践 / 生产化前检查
- 可信度等级：A
- 是否已验证：两个官方 Markdown 页面均返回 HTTP 200；关键段落已精读；可支撑 moderation、red-teaming、HITL、用户举报、safety identifiers、API key revoke、abuse monitoring logs、application state、Zero Data Retention / Modified Abuse Monitoring、endpoint retention、data residency 和第三方工具数据边界；标准库 production safety / data governance checklist audit 已完成；Real Moderation Safety harness 已准备并接入统一 runner，当前无 API key 只验证 skip 分支；真实 moderation 效果、合规适配、数据驻留配置和误用检测效果仍部分验证
...
- 已准备 Real Moderation Safety harness，用于后续记录 moderation `flagged`、categories、scores、latency、expected mismatch 和 policy decision；当前无 API key 时只验证 skip 分支，不证明真实检测效果。

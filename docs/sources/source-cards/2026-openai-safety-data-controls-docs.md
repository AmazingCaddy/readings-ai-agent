...
- 适合阶段：工程实践 / 生产化前检查
- 可信度等级：A
- 是否已验证：两个官方 Markdown 页面 2026-07-12 均返回 HTTP 200；关键段落已复核；可支撑 moderation、red-teaming、HITL、用户举报、safety identifiers、API key revoke、abuse monitoring logs、application state、Zero Data Retention / Modified Abuse Monitoring、endpoint retention、prompt caching application state、hosted container state、remote MCP / third-party transmission、data residency 和第三方工具数据边界；标准库 production safety / data governance checklist + object-level data-flow audit 已完成，覆盖 remote MCP、hosted execution、file/vector store、prompt caching 和 browser/computer-use 字段模板；Real Moderation Safety harness 已完成本地 deterministic policy-signal control，当前无 API key 未验证真实 API；真实 moderation 效果、合规适配、数据驻留配置、对象删除、trace 脱敏和误用检测效果仍部分验证
...
- Data controls 文档说明 prompt caching 可能把 encrypted key/value tensors 作为 GPU-local application state 暂存，通常不应把 prompt caching 误解成“没有任何中间状态”。
- Data controls 文档还说明传输到第三方服务的数据受第三方 retention policies 约束，data residency 也不覆盖 OpenAI 之外的第三方产品、服务或内容。
- Data controls 文档说明 data residency 是 project configuration，且不适用于 system data；某些地区、endpoint、模型和特性有额外限制。

## 支撑证据

- 2026-07-12 复核：`safety-best-practices.md` 返回 HTTP 200；`last-modified: Sat, 11 Jul 2026 12:01:07 GMT`；`content-type: text/markdown; charset=utf-8`。
- 2026-07-12 复核：`your-data.md` 返回 HTTP 200；`last-modified: Sat, 11 Jul 2026 07:17:19 GMT`；`content-type: text/markdown; charset=utf-8`。
...
- Data controls 页面包含 data not used for training by default、abuse monitoring logs、application state、Modified Abuse Monitoring、Zero Data Retention、organization/project data retention controls、endpoint retention matrix、prompt caching application state、remote MCP third-party retention、third-party network transmission、hosted container state、data residency controls 和 limitation sections。
...
- 已完成标准库 production safety / data governance checklist + object-level data-flow audit，验证 `safety_identifier`、API key revoke、abuse logs / application state、remote MCP third-party retention、hosted tool state、file/vector store 对象、prompt caching prefix、browser/computer-use 数据面、data residency 和 red-team / 用户举报回流可以拆成可审计字段。
- 已准备 Real Moderation Safety harness；无 API key 时已完成本地 deterministic policy-signal control，记录 moderation `flagged`、categories、scores、`category_applied_input_types`、expected mismatch 和 policy decision 分支，但不证明真实检测效果。
...
- 后续真实 MCP / hosted tool / file-vector-store / prompt-caching / browser-computer-use 实验应记录哪些数据发给第三方 server、哪些数据进入 application state、哪些对象需要显式删除、trace 如何脱敏、是否有 project-level data retention 或 data residency 配置。

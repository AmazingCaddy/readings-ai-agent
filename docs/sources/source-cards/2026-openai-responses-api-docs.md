...
- 适合阶段：入门 / 工程实践
- 可信度等级：A
- 是否已验证：来源链接已复核；关键字段已抽样精读；输入输出结构化接口边界可入正文；工具调用与上下文工程相关结论已交叉验证；真实运行行为仍部分验证
...
- 2026-07-11 复核同一页面中 message roles、`context_management`、`truncation`、`text.format`、`json_schema` 和 refusal 字段；已与 Text Generation 和 Structured Outputs 文档交叉验证第 02 章上下文工程边界，并支撑“LLM 应用输入输出不只是字符串”的窄结论。
...
- 结论：进入
- 原因：LLM 基础、工具调用和实践项目都需要当前官方 API reference；可与 Text Generation 文档共同支撑输入、输出、tool call、refusal、structured output 和 context management 的结构化接口边界。

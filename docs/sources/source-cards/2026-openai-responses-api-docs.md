...
- 已准备三个最小 Responses API harness：tool-calling 参数校验/重试、Structured Outputs / JSON mode 对比、prompt injection / permission。当前无 API key 时，Structured Outputs harness 已完成本地 deterministic schema/semantic control；tool-calling 和 prompt injection / permission 仍保守返回 `skipped`。配置 API key 后可记录输入、输出、工具调用、错误处理、权限策略和 trace。真实 API completed run 仍待跑，不能提前升级结论。

...
- 已完成标准库 trace-aware eval 最小实验；已准备真实模型 trace-aware eval harness 并记录无 API key skipped 结果，可在配置 API key 后记录 tool call/result/error/approval/final response 并比较 final-only 与 trace-aware scoring；后续仍需扩展为真实客服/RAG/tool traces，记录 LLM call、tool execution、retrieval、latency、token usage、session id，并做 Phoenix 平台对照。

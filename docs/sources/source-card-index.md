...
| [OpenAI Evals repo](source-cards/2026-openai-evals-repo.md) | Eval / Source Code | Source Code | A | README 和 custom eval 方向已精读；trace-aware eval 和 trace schema audit 已完成；工具/副作用 Agent 的 trace-aware eval 与 trace 字段用途设计窄边界可入正文；真实模型 trace harness 已准备 | 实际运行真实 eval harness、LLM-as-judge 误判和人工复核实验 |

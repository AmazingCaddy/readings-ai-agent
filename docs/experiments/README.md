...
当前 runner 状态见 [Validation Harness Runner 结果](validation-harness-runner/results-2026-07-12.md)：2026-07-12 运行覆盖 29 个入口，Real Tool Calling、Real Structured Outputs / JSON Mode、Real Prompt Injection / Permission、Real RAG Citation Synthesis、Real Trace-Aware Eval、Real Production Cost / Latency / Rate-Limit、Real Batch / Flex / Prompt Caching 和 Real Moderation Safety 在无 `OPENAI_API_KEY` 时完成本地 deterministic control，其余真实 API harness 保守跳过；OpenAI Agents SDK guardrail harness 通过临时依赖完成 input / output / tool input / tool output guardrail run，LlamaIndex harness 通过临时依赖完成本地 `VectorStoreIndex` / retriever / QueryEngine / `MockLLM` / source-node metadata run，Playwright harness 通过临时依赖和本地 Chromium headless shell 完成固定 demo page workflow 与 deterministic computer-use-style loop，LangGraph interrupt recovery harness 通过临时依赖完成 `MemorySaver` 最小 run 和 `SqliteSaver` 本地 SQLite 同进程 graph 重建恢复 case、双本地 Python 进程 prepare/resume case 和双本地 Python 进程并发 resume case，LangGraph memory store harness 完成本地 `InMemoryStore` namespace / put / get / search / delete run，Real Framework Same-Task Comparison 在标准 full runner 中完成 OpenAI Agents SDK / LangGraph / LlamaIndex adapter，Real smolagents validation 已有单独 `uv run --with smolagents` completed run，Real Multi-Agent Framework Validation 在标准 full runner 中完成 LangGraph adapter 并跳过 AutoGen/CrewAI adapter，本地 MCP stdio harness、官方 MCP Python SDK stdio harness、Claim Boundary Consistency Audit、Chapter Evidence Alignment Audit、Claim To Chapter Landing Audit、Source Reference Integrity Audit、Source To Claim Support Audit 和 Source Card Evidence Quality Audit 完成。runner 现在会把 `all_passed=false` 的 harness 计为失败，防止文本 audit 静默回归；Source Reference Integrity Audit 现在还检查 source-card 目录页和 MkDocs Source Cards 导航，当前共 10 项检查。Semantic Kernel plugin harness、same-task comparison 的 Semantic Kernel adapter、Real smolagents validation、Real Repo Issue Agent Toy、Real mini-SWE-agent CLI Surface Validation、Real mini-SWE-agent Runtime Surface Validation 和 Real Browser Use Package Surface Validation 均已有单独 completed run；在标准 full runner 中未安装对应依赖时保守 skipped。Real Multi-Agent Framework Validation 也有单独 AutoGen/CrewAI/LangGraph 全 completed run。Source URL Availability Audit 是联网快照，见 [2026-07-12 结果](source-url-availability-audit/results-2026-07-12.md)，本次 65 张 source cards 的 92 个主 URL 中 91 个返回 HTTP 2xx/3xx，attention items 为 1：`academy.langchain.com` 返回 HTTP 403；LangChain Academy GitHub / raw README、OpenAI Evals repo URL、LangChain / LangSmith docs URL 和 tau2/tau3-bench current repo URL 返回 HTTP 200。它不接入默认 runner，以免外部网络波动影响常规验证。
...
30. Real smolagents validation 实验
    - 目标：用 deterministic fake models 和本地 toy tools 验证 smolagents `CodeAgent` / `ToolCallingAgent` 的最小 runtime surface、`Tool.output_schema` 非强制校验和 default local executor import allowlist 表面。
    - 状态：真实 harness 已完成单独本地 run，见 [Real smolagents validation](real-smolagents-validation/README.md) 和 [2026-07-12 结果](real-smolagents-validation/results-2026-07-12.md)。本次用 `smolagents==1.26.0` 观察到 `CodeAgent` 执行 Python snippet 调用 toy tools，`ToolCallingAgent` 执行 structured tool calls，声明 `output_schema` 的 tool 返回 plain string 时没有 runtime output validation，默认 local executor 阻断未授权 `import os`，trace 未泄露 untrusted fixture secret。它不证明真实模型行为、code-agent benchmark、远程 sandbox 隔离、Hub/MCP tool 安全、成本、延迟或生产可靠性。

31. Real OpenAI Agents SDK guardrail validation 实验
...
32. Voyager-style toy embodied agent 实验
...
33. Claim boundary consistency audit 实验
...
34. Source reference integrity audit 实验
   - 目标：检查 source-card index、source card 元数据、章节 references 和本地 Markdown 链接的结构完整性，防止资料卡片漏索引、章节缺 references 或本地引用断链。
   - 状态：已完成标准库文本审计，见 [Source Reference Integrity Audit](source-reference-integrity-audit/README.md) 和 [2026-07-12 结果](source-reference-integrity-audit/results-2026-07-12.md)。本次检查 65 张 source cards、13 个编号章节和 350 个本地 Markdown links，全部通过。它不联网检查外部 URL，也不验证资料内容、章节解释、真实模型/API/框架行为、成本、延迟、安全或生产可靠性。

35. Source URL availability audit 实验
   - 目标：对 source cards 元数据中的主来源 URL 做活网络快照，确认当前环境能访问原始资料入口。
   - 状态：已完成联网快照，见 [Source URL Availability Audit](source-url-availability-audit/README.md) 和 [2026-07-12 结果](source-url-availability-audit/results-2026-07-12.md)。本次检查 65 张 source cards 的 92 个主 URL，91 个返回 HTTP 2xx/3xx，attention items 为 1：`academy.langchain.com` 返回 HTTP 403；LangChain Academy GitHub / raw README、OpenAI Evals repo URL、LangChain / LangSmith docs URL 和 tau2/tau3-bench current repo URL 返回 HTTP 200。它是手动/目标审计，不接入默认 runner；结果只证明本次网络环境下 URL 可达状态和 attention，不验证资料内容、章节解释、真实模型/API/框架行为、成本、延迟、安全或生产可靠性。

36. Source card evidence quality audit 实验
   - 目标：检查进入正文的 source cards 是否保留支撑证据、风险段落和边界措辞，防止 source card 从“可追溯证据”退化成无证据断言。
   - 状态：已完成标准库文本审计，见 [Source Card Evidence Quality Audit](source-card-evidence-quality-audit/README.md) 和 [2026-07-12 结果](source-card-evidence-quality-audit/results-2026-07-12.md)。本次检查 65 张 source cards，其中 40 张进入正文的 source cards 均具备 `支撑证据`、可观察 evidence marker、`可能的问题` 和边界措辞；同时修正了 MCP servers repo、LlamaIndex examples repo 和 Anthropic Building effective agents source card 的边界表述，并确认新增 Hugging Face Agents Course、LangChain Academy、smolagents 与 tau2/tau3-bench source cards 保留 partial-entry 边界。它不验证外部资料事实、章节解释、真实模型/API/框架行为、成本、延迟、安全或生产可靠性。

37. Chapter evidence alignment audit 实验
...
38. Claim to chapter landing audit 实验
...
39. Source to claim support audit 实验
   - 目标：检查 01-11 内容章节直接引用的 source cards 是否也出现在 claim ledger `支撑资料` 列中，防止章节引用资料但台账没有纳入证据链。
   - 状态：已完成标准库文本审计，见 [Source To Claim Support Audit](source-to-claim-support-audit/README.md) 和 [2026-07-12 结果](source-to-claim-support-audit/results-2026-07-12.md)。本次检查 11 个内容章节直接引用的 64 张 source cards，全部能匹配 claim ledger 支撑资料列；该 audit 不扫描第 12 章 source map，以免把全量资料索引误当正文引用。它不验证外部资料事实、章节解释、真实模型/API/框架行为、成本、延迟、安全或生产可靠性。

...
35. Source URL availability audit 实验
   ...
   - 状态：已完成联网快照，见 [Source URL Availability Audit](source-url-availability-audit/README.md) 和 [2026-07-12 结果](source-url-availability-audit/results-2026-07-12.md)。本次检查 64 张 source cards 的 89 个主 URL，86 个返回 HTTP 2xx/3xx，3 个 LangChain / LangSmith docs URL 返回 `URLError` 并记录为 attention；tau2/tau3-bench current repo URL 返回 HTTP 200。它是手动/目标审计，不接入默认 runner；结果只证明本次网络环境下 URL 可达状态和 attention，不验证资料内容、章节解释、真实模型/API/框架行为、成本、延迟、安全或生产可靠性。

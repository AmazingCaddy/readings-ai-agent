...
33. Source reference integrity audit 实验
   ...
   - 状态：已完成标准库文本审计，见 [Source Reference Integrity Audit](source-reference-integrity-audit/README.md) 和 [2026-07-12 结果](source-reference-integrity-audit/results-2026-07-12.md)。本次检查 62 张 source cards、13 个编号章节和 340 个本地 Markdown links，全部通过。它不联网检查外部 URL，也不验证资料内容、章节解释、真实模型/API/框架行为、成本、延迟、安全或生产可靠性。

34. Source URL availability audit 实验
   - 目标：对 source cards 元数据中的主来源 URL 做活网络快照，确认当前环境能访问原始资料入口。
   - 状态：已完成联网快照，见 [Source URL Availability Audit](source-url-availability-audit/README.md) 和 [2026-07-12 结果](source-url-availability-audit/results-2026-07-12.md)。本次检查 62 张 source cards 的 86 个主 URL，86 个返回 HTTP 2xx/3xx，0 个 attention；新增 Hugging Face Agents Course URL 返回 HTTP 200。它是手动/目标审计，不接入默认 runner；结果只证明本次网络环境下 URL 可达状态和 attention，不验证资料内容、章节解释、真实模型/API/框架行为、成本、延迟、安全或生产可靠性。

35. Source card evidence quality audit 实验
   - 目标：检查进入正文的 source cards 是否保留支撑证据、风险段落和边界措辞，防止 source card 从“可追溯证据”退化成无证据断言。
   - 状态：已完成标准库文本审计，见 [Source Card Evidence Quality Audit](source-card-evidence-quality-audit/README.md) 和 [2026-07-12 结果](source-card-evidence-quality-audit/results-2026-07-12.md)。本次检查 62 张 source cards，其中 39 张进入正文的 source cards 均具备 `支撑证据`、可观察 evidence marker、`可能的问题` 和边界措辞；同时修正了 MCP servers repo、LlamaIndex examples repo 和 Anthropic Building effective agents source card 的边界表述，并确认新增 Hugging Face Agents Course source card 保留 partial-entry 边界。它不验证外部资料事实、章节解释、真实模型/API/框架行为、成本、延迟、安全或生产可靠性。
...
38. Source to claim support audit 实验
   ...
   - 状态：已完成标准库文本审计，见 [Source To Claim Support Audit](source-to-claim-support-audit/README.md) 和 [2026-07-12 结果](source-to-claim-support-audit/results-2026-07-12.md)。本次检查 11 个内容章节直接引用的 61 张 source cards，全部能匹配 claim ledger 支撑资料列；该 audit 不扫描第 12 章 source map，以免把全量资料索引误当正文引用。它不验证外部资料事实、章节解释、真实模型/API/框架行为、成本、延迟、安全或生产可靠性。

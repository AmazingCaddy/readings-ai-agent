# Real Agentic Security Regression Set

## 目标

把 MITRE ATLAS 和 OWASP Agentic AI resources 中的 agentic-specific 风险，整理成可迁移到真实模型 / 真实框架的安全 regression set。当前已完成一个标准库 toy runtime，用来验证 case matrix、记录字段和失败统计；它不是为了证明某个防护方案安全，而是为了定义：哪些 case 应该被测试，如何记录误报、漏报、审批、trace 和副作用。

## 实验边界

当前包含一个标准库 toy runtime，用于验证 case matrix、记录字段和几种策略的失败模式。它尚未运行真实模型、真实框架、真实 MCP server、真实 browser/computer-use 工具或真实外部系统。
...
```bash
uv run python docs/experiments/real-agentic-security-regression-set/agentic_security_regression_set.py
```
...
- 当前状态：标准库 toy runtime 已完成；真实模型 / 真实框架实验待跑。
...
1. 迁移到 `real-prompt-injection-permission` harness，记录真实模型是否提出相关 tool calls。
2. 对 MCP、LangGraph interrupt、browser/computer-use 类 case 拆成独立真实框架实验，避免一个 harness 同时引入过多依赖。
3. 对 detector-only、policy-enforced、HITL 和 sandboxed runtime 记录真实成本、延迟和人工负担。

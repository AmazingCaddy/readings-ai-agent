...
用同一个本地确定性研究/复核任务验证 AutoGen AgentChat 和 CrewAI 的真实 Python runtime surface：角色化 agent、team/crew 编排、终止/任务输出、缺证据复核和脱敏 trace。
...
如果缺少某个依赖，对应 adapter 会返回 `skipped`，其他 adapter 仍可运行。统一 runner 默认不会安装 AutoGen/CrewAI，因此 full runner 中该实验可保守跳过。
...
- 当前状态：已完成 AutoGen AgentChat 0.7.5 和 CrewAI 1.15.2 的本地 fake-model runtime run，见 [2026-07-12 结果](results-2026-07-12.md)。
- 可支撑：多 Agent 框架可以提供 agent/team/task/crew 编排表面和可检查 transcript / task output；但证据覆盖、缺证据 rubric、审批策略、trace 脱敏和是否值得引入多 Agent 仍需要应用层设计和评估。
- 不能支撑：不能证明 AutoGen 或 CrewAI 在真实模型任务中更好、更安全、更便宜、更快，也不能证明真实冲突解决、token/latency/cost、生产 observability、部署恢复或人工评审负担。

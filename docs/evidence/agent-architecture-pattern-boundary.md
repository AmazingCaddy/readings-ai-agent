...
- Source 14：[Building effective agents](../sources/source-cards/2024-anthropic-building-effective-agents.md)
...
- 2026-07-12 复核：三篇核心论文的 arXiv 页面均返回 HTTP 200，arXiv API 分别确认 ReAct `2210.03629v3`、Reflexion `2303.11366v4`、Tree of Thoughts `2305.10601v2` 的 published / updated metadata、primary category、authors、comment 和摘要；ReAct 项目站点与 ToT 代码仓库也可访问。该复核增强来源可追溯性，但不新增真实模型、真实框架或生产任务效果证据。
...
- 一致点：Anthropic `Building effective agents` 给出从 augmented LLM 到 prompt chaining、routing、parallelization、orchestrator-workers、evaluator-optimizer、agents 的 pattern taxonomy；这些模式横跨固定 workflow、受控编排和更开放 agent loop，适合放入初学者由浅入深的架构路线。
- 一致点：Anthropic 文章强调 agents 需要从 tool results 或 code execution 获得 environment ground truth，可能在 checkpoints / blockers 暂停请求 human feedback，并且需要 stopping conditions、sandbox testing 和 guardrails。这补强“高自治 agent loop 必须有反馈、权限、停止条件和 trace”的工程边界。
...
- 边界：Anthropic 文章是官方工程经验总结，不提供可复现实验数据、样本分布或 benchmark；它能支撑 pattern 命名、适用条件和 guardrail / sandbox / stop-condition 边界，不能证明任何模式真实更快、更便宜或更可靠。
...
- 边界：论文摘要中的效果提升来自特定任务、模型、反馈来源和 baseline；框架文档说明能力和抽象，不等于提供严格对照实验。因此正文只能写成“适用方向和风险边界”，不能写成“复杂架构总能提升可靠性”。
...
- 可入正文：窄结论“ReAct 的核心思想是交替生成 reasoning traces 和 task-specific actions，用推理轨迹跟踪/更新计划、处理异常，用行动连接知识库或外部环境”已完成 2026-07-12 arXiv API 复核和交叉验证。ReAct 摘要直接支撑 reasoning / acting interleaving；标准库 workflow / hybrid / ReAct-like 对比实验支撑把 ReAct-like tool loop 作为控制流形态解释，但不支撑 ReAct 默认优于 workflow。
- 可入正文：窄结论“Tree of Thoughts 支持搜索式推理路径，但不等同于生产 Agent 编排框架”已完成 2026-07-12 arXiv API 复核和交叉验证。ToT 摘要直接支撑多个中间 thought、不同推理路径、自我评估、前瞻和回溯；LangGraph / workflow evidence 支撑把工程编排、状态持久化和 HITL 与研究式搜索推理区分开。
- 可入正文：窄结论“复杂 Agent 架构不是默认更可靠；是否引入 ReAct、Planner / Executor、Reflection、Tree of Thoughts、状态图、embodied lifelong learning、workflow patterns 或多 Agent，需要看任务边界、trace、成本、失败原因、权限和实验结果”已完成第一轮交叉验证。论文支撑不同研究模式，Anthropic `Building effective agents` 补强 augmented LLM、prompt chaining、routing、parallelization、orchestrator-workers、evaluator-optimizer 和 agents 的工程模式梯度，以及 sandbox、guardrails、human feedback checkpoint 和 stopping condition 边界；Voyager 补强 automatic curriculum / skill library / environment feedback 的开放式具身 Agent 机制，标准库 Voyager-style toy environment 进一步验证了 curriculum task、environment feedback、execution error、self-verification、verified skill library、sandbox rejection 和 stop condition 的最小 trace 形状；LangGraph 和 OpenAI Agents SDK 支撑现代工程编排、状态和 trace 边界，LangGraph current docs / historical examples 补强 state graph、planner/replan 和 HITL 的代码形态；多组标准库实验显示复杂控制结构会带来额外工具调用、错误传播、错误反思、重复读取或冲突风险。
...
- 可入正文：窄结论“Reflection / Reflexion 可以把任务反馈和文字反思用于后续尝试，但反思必须绑定可校验反馈、范围控制和 trace；未验证反思不应直接写入长期记忆或后续策略”已完成 2026-07-12 arXiv API 复核和交叉验证。Reflexion 摘要支撑 linguistic feedback、reflective text 和 episodic memory buffer 的研究机制；标准库 reflection/retry 实验显示 verified feedback 可以补齐 missing evidence，而 unverified reflection memory 会让错误重复。

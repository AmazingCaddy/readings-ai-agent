...
| P0 | 真实 prompt injection + tool permission / HITL / detector | OpenAI Responses API 或一个轻量 tool agent；可扩展 Prompt Shields、Anthropic-style tool-output screening、LangGraph interrupt/checkpointer 或同类检测层 | 攻击样例、raw tool output / JSON-encoded tool result / detector-only / policy-enforced / HITL 对照、阻断/漏报/误报、审批、pause/resume、幂等执行、参数快照和 trace 脱敏结果；harness 已准备，结果待跑；标准库审批恢复实验已完成；LangGraph docs 已确认 interrupt/resume、checkpointer、`thread_id`、node restart 和 side-effect idempotency 边界；Real LangGraph Interrupt Recovery 实验设计已准备 | 不证明 guardrail、screening、interrupt 或检测层完全安全，只能记录覆盖范围 |

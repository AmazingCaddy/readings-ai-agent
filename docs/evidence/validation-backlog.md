...
- 成本、延迟和限流应该如何进入 Agent eval / production gate？窄结论已可入正文：上线前应记录 token / usage、request/token rate limits、rate-limit headers、retry、平均/P95 latency、model choice、budget threshold 和降级策略。OpenAI Production / Cost / Latency / Rate Limit docs 已补官方边界；仍需真实 API / Cookbook 最小练习验证具体成本、延迟、吞吐、质量取舍和优化效果。
...
- 生产化练习中应如何记录成本、延迟和限流？OpenAI Production / Cost / Latency / Rate Limit docs 已补官方 source card，可支撑 usage dashboard、token counting、rate-limit headers、exponential backoff、`max_tokens`、streaming、batching、项目隔离和预算阈值的工程边界；仍需真实练习记录 API key / project 设置、请求日志、rate-limit response、retry 行为、平均/P95 latency、cost estimate 和预算阻断体验。

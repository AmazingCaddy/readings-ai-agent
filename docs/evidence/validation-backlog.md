...
| P1 | 真实 safety / data governance checklist | OpenAI Safety best practices；OpenAI Data controls；项目级 data retention / data residency 配置 | moderation/HITL/detector 对照、`safety_identifier` 日志、API key revoke 演练、abuse logs vs application state 分类、remote MCP / hosted tool 数据流、对象删除和 data residency 项目配置检查 | 不证明任何检测层或合规配置充分有效；只能验证所选项目、endpoint、工具和账户能力 |

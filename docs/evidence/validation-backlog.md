...
| P0 | 真实 prompt injection + tool permission / HITL / detector | OpenAI Responses API 或一个轻量 tool agent；可扩展 Prompt Shields、Anthropic-style tool-output screening、LangGraph interrupt/checkpointer 或同类检测层；agentic-specific cases 覆盖 goal hijacking、tool misuse、identity / privilege abuse、memory poisoning、insecure inter-agent communication、cascading failures 和 rogue agent / runaway loop 停止条件 | 攻击样例、raw tool output / JSON-encoded tool result / detector-only / policy-enforced / HITL 对照、阻断/漏报/误报、审批、pause/resume、幂等执行、参数快照、trace 脱敏、身份/权限/记忆/多 Agent 通信/级联失败 case 结果；harness 已准备，结果待跑；标准库审批恢复实验已完成；LangGraph docs 已确认 interrupt/resume、checkpointer、`thread_id`、node restart 和 side-effect idempotency 边界；Real LangGraph Interrupt Recovery 实验设计已准备 | 不证明 guardrail、screening、interrupt、检测层、runtime containment 或 agentic-specific 控制完全安全，只能记录覆盖范围 |
...
| P1 | 真实 Agent trace-aware eval | Responses API toy agent trace；后续扩展到 trace grading、datasets/eval runs、grader 对照、LangSmith / Phoenix；OpenAI Evals / graders platform 正在退役，平台入口需复核 | runs/traces、final-only vs trace-aware 对比、trace grading rubric、dataset/eval run 记录、规则 grader / LLM-as-judge / Python grader / 人工复核对照、reward hacking 样例；真实模型 trace harness 已准备，结果待跑 | 自动评分不能当真值，需保留人工抽样；只能验证所选任务、模型、grader 和平台字段 |
| P1 | 真实 production cost / latency / rate-limit 记录 | OpenAI API / Cookbook 最小练习；可从 Structured Outputs、tool calling 或 RAG 小项目开始 | input/output tokens、usage、rate-limit headers、retry count、平均/P95 latency、model choice、budget threshold、cost estimate、降级/停止策略和失败样例；OpenAI Production / Cost / Latency / Rate Limit docs 已复核 | 不证明任一优化默认有效；只能验证所选模型、任务、并发、网络和账户限流条件 |
| P1 | 真实 Batch / Flex / Prompt Caching 对照 | OpenAI Batch API、Flex processing、Prompt Caching | Batch status、completed/failed/expired、`custom_id` 映射、Flex timeout / `429 Resource Unavailable` / fallback、`cached_tokens`、`cache_write_tokens`、cache hit/miss、等待时间、成本估算和 latency | 不证明 Batch/Flex/Prompt Caching 默认更优；只能验证所选 workload、模型、缓存前缀、账户限流和等待时间约束 |
...
| P1 | 真实 safety / data governance checklist | OpenAI Moderation；OpenAI Safety best practices；OpenAI Data controls；项目级 data retention / data residency 配置 | moderation/HITL/detector 对照、tool arguments / tool outputs 覆盖检查、streaming moderation 时机、`safety_identifier` 日志、API key revoke 演练、abuse logs vs application state 分类、remote MCP / hosted tool 数据流、对象删除和 data residency 项目配置检查 | 不证明任何检测层或合规配置充分有效；只能验证所选项目、endpoint、工具和账户能力 |
...
- 工具调用权限应该如何设计确认边界？窄结论已可入正文：高风险工具需要系统/应用层最小权限、参数校验、人工确认、审批状态恢复、幂等执行和审计 trace 的组合，不能只靠模型自觉。标准库 prompt injection / tool permission、安全 regression set 和审批状态恢复实验已完成；LangGraph current docs 已确认 interrupt/resume、checkpointer、`thread_id`、node restart、interrupt 顺序和 side-effect idempotency 的机制边界；OWASP Agentic AI resources 已补 goal hijacking、tool misuse、identity / privilege abuse、memory poisoning、多 Agent 通信、级联失败和 rogue agent 等风险分类；仍需真实 Responses API / Agents SDK / LangGraph / MCP / Semantic Kernel 等框架实验验证 guardrail/HITL 覆盖范围、误报漏报、pause/resume、参数快照、trace 脱敏、agentic-specific regression cases 和跨框架差异。
...
- LLM-as-judge、online evaluator 和自动化规则的误判率、成本与抽样人工复核比例如何设计？OpenAI Evaluation best practices 已补 position bias、verbosity bias、pairwise/pass-fail 倾向、human labels / human feedback 校准等官方边界；OpenAI Graders docs 已补 score model grader、Python grader、tool-call grading、grader hacking / reward hacking 和执行约束边界；仍需真实任务中测误判、成本、延迟、reward hacking 和人工复核比例。
- 成本、延迟和限流应该如何进入 Agent eval / production gate？窄结论已可入正文：上线前应记录 token / usage、request/token rate limits、rate-limit headers、retry、平均/P95 latency、model choice、budget threshold 和降级策略。OpenAI Production / Cost / Latency / Rate Limit docs 以及 Batch / Flex / Prompt Caching docs 已补官方边界；仍需真实 API / Cookbook 最小练习验证具体成本、延迟、吞吐、缓存命中、质量取舍和优化效果。

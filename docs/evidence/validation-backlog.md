...
| P1 | 真实 production cost / latency / rate-limit 记录 | OpenAI API / Cookbook 最小练习；可从 Structured Outputs、tool calling 或 RAG 小项目开始 | 标准库 production cost / latency / rate-limit field audit 已完成，覆盖 input/output tokens、request count、cost estimate、rate-limit headers、bounded retry、平均/P95 latency、model/output controls、budget gate 和降级字段；仍需真实 API 记录 input/output tokens、usage、rate-limit headers、retry count、平均/P95 latency、model choice、budget threshold、cost estimate、降级/停止策略和失败样例 | 不证明任一优化默认有效；只能验证所选模型、任务、并发、网络和账户限流条件 |
...
- Browser Agent 应该如何评估？窄结论已可入正文：浏览器 / computer-use Agent 不能只看最终文本或 demo 成功，必须检查网页动作、页面/DOM/截图状态、登录态/profile、VM/container 隔离、文件上传、表单提交、购物/支付、权限确认、action validation、trace 和脱敏。Browser Use / Playwright / Anthropic Computer Use / WebArena 已完成第一轮复核；仍需固定 Playwright workflow vs Browser Use browser agent vs computer-use-style action loop 小实验。
...
- 成本、延迟和限流应该如何进入 Agent eval / production gate？窄结论已可入正文：上线前应记录 token / usage、request/token rate limits、rate-limit headers、retry、平均/P95 latency、model choice、budget threshold、Batch/Flex/Prompt Caching 观测字段和降级策略。OpenAI Production / Cost / Latency / Rate Limit docs 以及 Batch / Flex / Prompt Caching docs 已补官方边界；标准库 production cost / latency / rate-limit audit 已完成字段检查模板；仍需真实 API / Cookbook 最小练习验证具体成本、延迟、吞吐、缓存命中、质量取舍和优化效果。

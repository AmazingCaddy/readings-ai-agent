...
- 部分验证：RAG paper 支撑外部检索增强的基础动机，LlamaIndex 支撑现代工程流程和组件边界。仍缺本地最小实验和具体 citation/source 追溯实现验证。

## 可进入章节

- 是。可以写成：RAG 是一条工程 pipeline，而不是单个 prompt 技巧。初学者应先保证加载、切分、索引、检索、回答合成和评测可观察，再讨论更复杂的 rerank、hybrid retrieval 或 agentic RAG。

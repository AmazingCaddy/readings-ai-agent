...
- 2026-07-12 复核：RAG paper 和 Self-RAG paper 的 arXiv 页面均返回 HTTP 200，arXiv API 分别确认 RAG `2005.11401v4` 和 Self-RAG `2310.11511v1` 的 published / updated metadata、primary category、authors、comment 和摘要。该复核增强来源可追溯性，但不新增真实 embedding、vector store、LLM synthesis、File Search、citation correctness、成本或延迟证据。
...
- 一致点：OpenAI Retrieval guide 说明 vector store file 加入后会自动 chunk、embed 和 index，并给出默认 chunking 参数、chunking 限制、文件大小/token 限制、storage pricing、删除后 eventual consistency、per-vector-store ingest rate limit、batch ingestion、attributes 上限和 batch/per-file attributes + chunking override 边界。这支持第 06/11 章把托管 RAG 也纳入吞吐、成本、延迟、删除一致性、权限和数据治理验证。
- 一致点：OpenAI File Search guide usage notes 列出 Responses / Chat Completions / Assistants availability、tiered RPM、pricing、ZDR 和 data residency 链接。这支持实践项目把 rate limits、storage pricing 和数据保留/地域设置作为记录项，而不是只看答案文本。
...
- 边界：OpenAI File Search / Retrieval 是官方产品文档，支撑 API shape 和托管检索能力存在；不能证明默认 chunking/ranking、file citations、semantic search、batch ingestion 或 hosted tool 在具体业务中的 citation correctness、faithfulness、成本、延迟、删除一致性、账号配额或权限治理效果。
...
- 是否需要实验：是
- 实验设计：用同一组手册资料建立最小 RAG pipeline，比较不同 chunk size、metadata、top-k、rerank/filter、OpenAI File Search / vector store 和无 RAG baseline。记录检索命中率、included search results、引用正确率、答案忠实度、batch ingestion、rate-limit / ingest-limit 行为、延迟、token / storage 成本、ZDR/data residency 设置、删除一致性和无法回答时的处理。
...
- 可入正文：窄结论“RAG 的基础动机包括外部知识访问、知识更新和 provenance / source traceability”已完成 2026-07-12 arXiv API 复核和交叉验证。RAG paper 摘要直接支撑知识密集任务、外部检索、provenance 和 world knowledge 更新动机；LlamaIndex RAG 文档支撑现代工程中通过 loading / indexing / retrieval / response synthesis / evaluation 把外部数据接入 LLM 上下文。
- 可入正文：窄结论“工程 RAG 是 loading、indexing、storing、querying/retrieval、response synthesis 和 evaluation 等阶段组成的可观察 pipeline，不是单个 prompt 技巧；最小可治理 RAG 应保留 chunk metadata、retrieval trace、citation/source 绑定、检索必要性/相关性判断、citation verifier 和无证据拒答或 `grounded=false` 标记”已完成 2026-07-12 arXiv API 复核和交叉验证。RAG paper 支撑外部检索、provenance 和知识更新动机，Self-RAG paper 支撑不要盲目固定检索、需要评估 retrieval necessity / passage relevance / critique / citation accuracy，LlamaIndex docs 支撑现代工程流程和组件边界，LlamaIndex examples repo 补强 citation/source node、retriever 和 RAG workflow 的代码示例证据，OpenAI File Search / Retrieval docs 补强 hosted file search、vector stores、included search results、metadata filtering、ranking、chunking、expiration、batch ingestion、rate limits、pricing/ZDR/data residency 和成本边界，标准库最小 pipeline 实验复现了 chunk / retrieve / synthesize trace、chunk-level citations 和 unsupported question 拒答流程，Real RAG Citation Synthesis control 补强 citation id、quote matching、groundedness 和 unsupported grounded claim 的校验器失败样例，Real LlamaIndex run 补强真实框架 source-node / QueryEngine metadata plumbing 的最小观察。
- 部分验证：真实 RAG stack、OpenAI File Search / vector store、embedding、chunk size/top-k/rerank 对比、真实 LLM synthesis faithfulness、citation correctness、latency、token / storage cost、batch ingestion、rate-limit / ingest-limit 行为、删除一致性、ZDR/data residency 设置和生产权限边界仍需实验；不能写成某个检索、托管工具或 chunk 策略默认最优。本地 verifier control 不证明真实模型引用忠实性。

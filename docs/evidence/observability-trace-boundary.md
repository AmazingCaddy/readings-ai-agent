...
- Source 8：[Evidence Note: Agent Eval 与 Trajectory 边界](agent-eval-trajectory-boundary.md)
- Source 9：[Evidence Note: Browser Agent 与网页自动化边界](browser-agent-boundary.md)
- Source 10：[Trace-Aware Eval 最小实验结果](../experiments/trace-aware-eval/results-2026-07-11.md)
- Source 11：[Trace Schema Audit 最小实验结果](../experiments/trace-schema-audit/results-2026-07-11.md)
...
- 边界：OpenAI Evaluation guides 支撑 trace grading 和 eval workflow 的工程形态，但不证明任何 grader、judge model、平台 trace 或指标在具体业务中准确可靠；OpenAI Evals platform 退役时间也要求正文区分 eval 方法和具体平台入口。
...
- 可入正文：窄结论“对工具型或有副作用的 Agent，trace 是 eval、审计和回归输入，不只是 debug 日志”已完成第一轮交叉验证。论文/benchmark 支撑过程评测的重要性，OpenAI Evals repo / Evaluation guides 支撑 custom eval、trace grading、datasets/eval runs、continuous evaluation 和人工校准，LangSmith/Phoenix/Cookbook 支撑 trace、runs、spans、datasets、online/offline evaluation 和反馈工作流；标准库 trace-aware eval 支撑 trace 能发现 final-only 漏掉的过程错误。
...
- 部分验证：完整通用 trace schema、平台字段覆盖、真实 Agent / RAG traces、自动评分误判分析和人工复核设计仍需真实运行与平台对照；不能写成任何平台默认覆盖这些字段，也不能把这份字段清单写成所有 Agent 系统的唯一标准。OpenAI Evals platform 退役不影响 eval 方法本身，但会影响具体平台教程和工具入口。

...
| smolagents | `CodeAgent` code snippets、`ToolCallingAgent` JSON/text tool calls、tool metadata、Hub/MCP tools、local/remote code execution | code action vs structured tool calling 对照、sandbox / `trust_remote_code` / Hub-MCP tool 信任边界 | 不等于 code agents 默认更好；`LocalPythonExecutor` 不是 security boundary；`output_schema` 只作 informational use；Hub/MCP tools 可能下载并本地执行代码；API experimental | 同一任务 `CodeAgent` / `ToolCallingAgent` 对照、Docker/E2B/Blaxel/Modal sandbox、MCP/Hub tool trust review、真实模型成本/延迟/trace |

...
- 仍缺：同一真实任务在 2-3 个框架下的最小实现，再逐步扩展到完整横向比较。应记录依赖、实现时间、LOC、trace 可读性、权限/人工确认、错误恢复、token/latency/cost 和维护复杂度。

## 结论状态

- 可入正文：窄结论“框架比较应从任务难点、能力边界和约束出发，而不是从框架名、流行度或排行榜出发”已完成第一轮交叉验证。框架 source cards 支撑各自主轴定位；rubric smoke test 支撑任务画像式比较方法，并明确低分只表示任务不匹配，不表示框架差。
- 部分验证：真实框架横向实验仍缺，因此不能写成“某个框架默认最好”，也不能断言真实 tracing、permission、HITL、RAG citation、tool error recovery、deployment、latency、token cost 或维护复杂度表现。

...
- 一致点：Real Multi-Agent Framework Validation 显示，同一个 researcher/reviewer 缺证据复核任务可以用 AutoGen AgentChat 的 `AssistantAgent` + `RoundRobinGroupChat` + `TextMentionTermination` 和 CrewAI 的 `Agent` + `Task` + `Crew(process=sequential)` 表达。两者都能产生可检查 transcript / task output，但 fake model response、evidence policy、missing-evidence rubric 和 trace redaction 仍是应用层代码。
...
- 已完成：Real Multi-Agent Framework Validation 用同一个本地缺证据复核任务跑通 AutoGen AgentChat 和 CrewAI adapter，并记录 framework-owned / application-owned capabilities、transcript / task output、缺证据暴露和 trace 脱敏。
- 仍缺：真实模型、OpenAI Agents SDK hosted tracing / 真实 approval UI、部署式恢复、OpenAPI/MCP plugin、LangGraph multi-agent、实现时间、LOC、token/latency/cost 和维护复杂度的完整横向比较。

...
- 一致点：MITRE ATLAS latest v6 YAML 将 `AML.T0051` `LLM Prompt Injection`、`AML.T0053` `AI Agent Tool Invocation`、`AML.T0110` `AI Agent Tool Poisoning`、`AML.T0011.002` `Poisoned AI Agent Tool`、`AML.T0099` `AI Agent Tool Data Poisoning`、`AML.T0070` `RAG Poisoning` 和 `AML.T0034.002` `Agentic Resource Consumption` 放入可版本化的 technique 数据集中，并用 `Agentic AI` platform 与 `Feasible` / `Demonstrated` / `Realized` maturity 标注攻击技术成熟度。`AML.CS0035`、`AML.CS0040`、`AML.CS0045`、`AML.CS0046`、`AML.CS0053`、`AML.CS0054`、`AML.CS0055` 等 case-study relationships 可作为 RAG exfiltration、memory poisoning、MCP / remote tool poisoning 和 computer-use action regression case 的映射依据；这补强了“安全 regression set 应从真实攻击技术和案例抽样”的边界。
...
- 边界：MITRE ATLAS 是威胁知识库和 YAML 数据集，mitigation 条目只能作为 checklist 候选，case-study relationships 只能作为测试映射证据；它不能证明任何 guardrail、detector、sandbox、HITL 或监控方案在真实系统中有效，也不能证明攻击在读者系统中的发生频率、可利用性或复现成功率。

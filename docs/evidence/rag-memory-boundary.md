...
- 2026-07-12 复核：RAG paper arXiv 页面返回 HTTP 200，arXiv API 确认 `2005.11401v4`、published `2020-05-22T21:34:34Z`、updated `2021-04-12T15:42:18Z`、primary category `cs.CL`、Accepted at NeurIPS 2020 comment 和摘要。该复核增强 RAG / Memory 术语边界的来源追溯，但不新增真实 RAG 或 memory framework 行为证据。
...
- 可入正文：窄结论“RAG 偏外部知识检索、provenance 和知识更新；Agent memory 偏会话状态、跨会话数据和治理；RAG paper 中的 non-parametric memory 不应直接等同于 Agent long-term memory”由 2026-07-12 复核后的 RAG paper、LangGraph memory docs 和标准库 RAG / Memory 对比实验共同支撑。

...
- 部分验证：OpenAI Agents SDK 和 LangGraph 官方文档直接支撑 agent loop、runtime-managed workflow、stateful workflow/agent 和 orchestration 边界；ReAct 支撑推理-行动循环的研究脉络；标准库实验支撑固定 workflow、hybrid 和 tool loop 的最小比较流程。仍缺真实模型 / 框架 / repo issue 实验和更多框架横向资料。

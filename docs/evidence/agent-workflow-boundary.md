...
- 可入正文：窄结论“Agent 和 Workflow 不是互斥概念，而是控制权、状态推进、工具调用顺序和运行时决策方式的连续谱；实际系统可以组合成 workflow-agent hybrid”已完成第一轮交叉验证。OpenAI Agents SDK 和 LangGraph 官方文档直接支撑 agent loop、runtime-managed workflow、stateful workflow/agent 和 orchestration 边界；LangGraph current quickstart 补强显式 state/node/edge/conditional edge 的代码形态；ReAct 支撑推理-行动循环的研究脉络；标准库实验支撑固定 workflow、hybrid 和 tool loop 的最小比较流程。

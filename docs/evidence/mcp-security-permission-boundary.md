...
- 部分验证：官方 spec 和 security best practices 已直接支撑 MCP 安全、授权和权限边界的正文表述；标准库模拟实验和本地 stdio JSON-RPC harness 支撑最小 trace、tool approval、resource review 和进程消息边界设计。仍缺真实 MCP SDK / host trace、OAuth / URL mode / token redaction 实验和不同 host 实现差异对比。

...
- 结果：已完成标准库模拟实验。只读工具被批准，模拟写工具被 host policy 拒绝，普通 resource 被标记为 ordinary context，恶意 resource 被标记为 `untrusted_prompt_injection_candidate`。实验未覆盖真实 URL mode elicitation、OAuth authorization、token audience validation、真实 UI 或 SDK 行为。

## 结论状态

- 部分验证：官方 spec 和 security best practices 已直接支撑 MCP 安全、授权和权限边界的正文表述；标准库模拟实验支撑最小 trace、tool approval 和 resource review 字段设计。仍缺真实 MCP SDK / host trace、OAuth / URL mode / token redaction 实验和不同 host 实现差异对比。

## 可进入章节

- 是。可以写成：MCP 标准化了工具、资源、提示和部分授权/交互机制，但安全不是“协议一接就自动完成”；高风险能力仍需要最小权限、用户确认、token audience 校验、输入输出校验、sandbox、审计和真实 host/client/server 实验验证。

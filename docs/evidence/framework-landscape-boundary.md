...
- 边界：Semantic Kernel Process Framework 当前入口是 `frameworks/process/process-framework`，旧 `frameworks/process/` 路径已返回 404；Process Framework 仍标注 experimental，只能作方向性参考。CrewAI source card 可信度为 B；框架文档通常强调能力和产品定位，不等于严格对照实验。

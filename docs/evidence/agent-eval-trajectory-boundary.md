...
- 部分验证：AgentBench 和 WebArena 支撑交互环境、长程任务、工具/外部知识和失败原因分析的重要性；OpenAI Evals 支撑为具体 LLM 系统和 tool-using agents 设计 custom eval。仍缺本地实验和更具体的 observability/trace 工程资料。

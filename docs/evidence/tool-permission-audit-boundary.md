...
- 一致点：Semantic Kernel Plugins / task automation 文档区分 retrieval functions 和 task automation functions，并指出 task automation functions 往往需要 human-in-the-loop approval processes；2026-07-12 复核的 task automation 页面进一步展示 function invocation filter 可在函数调用前请求用户同意并把拒绝结果返回给 agent。这与 OpenAI HITL 机制方向一致，但仍只是实现模式，不证明审批状态恢复、幂等执行或真实 HITL UI 默认安全。
...
- 一致点：MITRE ATLAS latest v6 YAML 把 Agentic AI 定义为可自主 plan、decide、execute multi-step actions 的平台，并包含 `AML.T0053` `AI Agent Tool Invocation`、`AML.T0110` `AI Agent Tool Poisoning`、`AML.T0011.002` `Poisoned AI Agent Tool`、`AML.T0099` `AI Agent Tool Data Poisoning`、`AML.T0034.002` `Agentic Resource Consumption` 等 Agentic AI technique 线索；`AML.CS0045`、`AML.CS0053`、`AML.CS0054` 可映射 MCP / remote tool exfiltration，`AML.CS0040` 可映射 memory poisoning，`AML.CS0046` / `AML.CS0055` 可映射 computer-use destructive or hijacked actions。这补强了工具权限、远程工具、记忆、成本滥用和 computer-use action 都应进入安全 regression set 的边界。
...
- 边界：MITRE ATLAS mitigations 只能作为控制候选和 checklist 来源，case-study relationships 只能支撑测试用例映射；不能单独证明任何权限继承、工具确认、segmentation、memory hardening、validation、sandbox/runtime containment 或 monitoring 在真实 Agent 系统中有效。

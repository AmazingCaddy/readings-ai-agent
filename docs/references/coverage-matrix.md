...
3. 为 MCP、Tool Use、RAG、Eval 四个主题补齐源码 examples。MCP 已完成官方 spec、安全、授权第一轮和标准库 trace 模拟，仍需真实 MCP SDK / host trace 与权限实验；Tool Use 已完成参数校验/重试标准库模拟，仍需真实模型 / API 实验和框架对照；Eval 已完成 OpenAI Evals README、Cookbook、LangSmith、Phoenix、标准库 trace-aware eval 和真实模型 trace harness 第一轮，仍需实际运行真实 Agent trace、自动评分误判和人工复核实验。

...
| Planning / Orchestration | 待补 | Tree of Thoughts；Reflexion；LangGraph docs | Planner / Executor 对比实验；Reflection / Retry 实验；LangGraph docs | Planner/Executor 需要可执行计划、证据校验、失败反馈和重规划 trace 的窄边界可入正文；Reflection 需要可校验反馈、范围控制和 trace，未验证反思不应直接进入长期记忆或后续策略的窄边界可入正文；关键论文摘要、LangGraph 段落、标准库 planner/executor 和 reflection/retry 对比已完成；真实表现仍部分验证 | 需要状态机、真实模型和真实框架对比实验 |

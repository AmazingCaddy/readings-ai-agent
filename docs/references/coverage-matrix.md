...
| Planning / Orchestration | 待补 | Tree of Thoughts；Reflexion；LangGraph docs | Planner / Executor 对比实验；Reflection / Retry 实验；LangGraph docs | Tree of Thoughts 支持搜索式推理路径但不等同于生产编排框架、Planner/Executor 需要可执行计划/证据校验/失败反馈/重规划 trace、Reflection 需要可校验反馈/范围控制/trace 的窄边界可入正文；关键论文摘要、LangGraph 段落、标准库 planner/executor 和 reflection/retry 对比已完成；真实表现仍部分验证 | 需要状态机、真实模型和真实框架对比实验 |
...
| 框架生态 | 待补 | OpenAI Agents SDK；LangGraph；LlamaIndex；AutoGen；Semantic Kernel；CrewAI | 框架能力交叉表；框架选择 rubric smoke test；官方 examples 候选 | “框架应按任务难点和能力边界比较，不能写成某个框架默认最好”的窄边界可入正文；关键框架定位已精读；框架能力交叉表和标准库 rubric smoke test 已完成；真实横向表现仍部分验证 | 需要真实同一任务横向实验、observability/permission 对比和官方 examples |

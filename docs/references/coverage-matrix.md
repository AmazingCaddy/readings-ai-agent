...
| --- | --- | --- | --- | --- | --- |
| AI Agent 定义与边界 | 待补 | OpenAI Agents SDK docs；LangGraph docs | 待补 | 候选 | 需要官方定义和 workflow 对比来源 |
| LLM 与上下文工程 | OpenAI docs 候选 | OpenAI Responses API docs | OpenAI cookbook 候选 | 候选 | 需要结构化输出和上下文失败模式资料 |
| Tool Use / Function Calling | OpenAI tool calling docs 候选 | Toolformer paper；OpenAI docs | OpenAI cookbook 候选 | 部分链接已复核 | 需要现代 API 文档精读 |
| Agent 架构模式 | 待补 | ReAct paper；Reflexion paper；Tree of Thoughts paper | LangGraph examples 候选 | 部分链接已复核 | 需要 workflow-agent hybrid 工程来源 |
| MCP | MCP official docs | MCP official docs | MCP example servers 候选 | 部分链接已复核 | 需要官方 spec 细读和安全边界资料 |
| RAG | 待补 | RAG paper；Self-RAG 候选 | LlamaIndex examples 候选 | 部分链接已复核 | 需要现代 RAG 工程实践和 memory 对比 |
| Memory / 知识库治理 | 待补 | 待补 | 待补 | 缺口较大 | 需要长期记忆、写入守门、冲突治理资料 |
| Planning / Orchestration | 待补 | Tree of Thoughts；Reflexion | LangGraph docs 候选 | 部分链接已复核 | 需要 planner/executor 和状态机工程资料 |
...
| Production / 安全 / 成本 | 待补 | OWASP LLM Top 10；NIST AI RMF 候选 | 框架安全文档候选 | 候选 | 需要 prompt injection、权限、审计资料 |
...
2. 精读并提取已经链接复核的 9 个来源。
3. 为 MCP、Tool Use、RAG、Eval 四个主题补齐官方文档和工程 examples。
4. 为 Memory / 知识库治理单独调查资料，这是当前最大缺口。
5. 为 Production / Security 建立独立资料组，避免 Agent 教程忽略安全边界。

...
这些资料不证明某个应用会便宜或快。减少输出 token、设置更贴近期望输出的 `max_tokens`、选择更小模型、streaming、batching、Batch API 或 flex processing 都是需要用真实任务 eval 复核的取舍，而不是默认优化。
...
- 成本和延迟应该如何纳入 Agent eval？已补 OpenAI 官方 production / rate limit / cost / latency / token counting 边界，仍需真实 API / Cookbook 练习记录 token、usage、rate-limit headers、retry、平均/P95 latency、cost estimate、budget threshold 和失败样例。

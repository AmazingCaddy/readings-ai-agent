...
- OpenAI Safety / Data Controls 文档已补充安全和数据治理资料，可支撑 moderation、red-team、HITL、输入/输出约束、用户举报、`safety_identifier`、API key revoke、abuse monitoring logs、application state、ZDR/MAM、endpoint retention、remote MCP third-party retention、hosted container state 和 data residency 的工程边界。它不证明任意检测层、数据控制配置或合规方案充分有效。

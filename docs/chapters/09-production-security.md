...
更具体地说，ATLAS 2026.06 YAML 中的 `AML.T0053` 可映射到 agent tool invocation case，`AML.T0110` / `AML.T0011.002` 和 `AML.CS0053` / `AML.CS0054` 可映射到 tool poisoning / MCP case，`AML.CS0040` 可映射到 memory poisoning case，`AML.CS0046` / `AML.CS0055` 可映射到 computer-use 高风险动作 case，`AML.T0070` / `AML.CS0035` 可映射到 RAG poisoning / data exfiltration case，`AML.T0034.002` 可映射到 resource / cost abuse case。这些 ID 支撑的是 case-study-derived regression set 设计，不证明攻击在你的系统中一定可复现，也不证明任何 guardrail、detector、HITL、sandbox 或 monitoring 有效。
...
以 LangGraph 这类 graph runtime 为例，当前文档把 human-in-the-loop 做成 `interrupt()` / resume 机制：流程可以在关键节点暂停，把需要人类判断的数据交给调用方，再用 resume command 继续；event streaming 可以暴露 `stream.interrupted` / `stream.interrupts`。这个机制适合解释“审批点应在工作流里显式建模”，但初学者要特别注意它的限制：恢复时 node 可能从头重新执行，`interrupt()` 前的代码会再次运行；有副作用的代码应该放在确认之后，或设计成幂等；多个 interrupt 要按 ID 配对恢复；恢复依赖正确的 checkpointer 和同一个 `thread_id`。内存型 checkpointer 只适合教程或进程内实验，进程重启会丢 checkpoint；长会话还要治理 checkpoint 增长、保留和清理。
...
AgentDojo 进一步补强了工具型 Agent 安全评测的形状：测试不应只检查最终回答，还应把用户任务、攻击者目标、注入端点、可用工具、环境状态、utility check 和 security check 放在同一个 stateful environment 里。它的论文 suite 覆盖 Workspace、Slack、Travel、Banking 四类环境，主文报告 70 个工具、97 个用户任务、27 个注入目标和 629 个 security test cases。对初学者来说，最重要的不是记住这些数字，而是学习评测结构：外部工具结果是非可信数据，工具调用会改变状态，防御也要用自适应攻击和误报/漏报来检验。论文中的 detector、重复用户 prompt 和 tool filtering 结果只适用于其 suite / setting，不能写成生产防护充分有效。

CaMeL / Defeating Prompt Injections by Design 则补强了另一类防护思路：不要只要求模型“不要听外部内容里的指令”，而是把可信用户请求中的 control flow 和 data flow 抽取出来，让后续检索到的非可信数据不能改变程序流程；同时用 capability 和 security policy 限制 private data 通过未授权工具调用外流。这个方向适合帮助初学者理解为什么“外部内容是数据，不是命令”需要落实到程序结构和工具权限，而不只是写进提示词。它仍是研究资料：README 明确代码是 research artifact，可能有 bug、可能不 fully secure，不是受支持产品；论文分数也只属于 AgentDojo / paper setting，不能写成生产防护默认有效。
...
这些资料不证明某个应用会便宜或快。减少输出 token、设置更贴近期望输出的 `max_tokens`、选择更小模型、streaming、batching、Batch API、flex processing 或 prompt caching 都是需要用真实任务 eval 复核的取舍，而不是默认优化。尤其要注意：Batch 适合不需要即时响应的离线任务；Flex 仍是 beta，适合能接受慢响应和偶发资源不可用的低优先级任务；Prompt Caching 要记录 `prompt_cache_key`、explicit breakpoints、`cached_tokens`、`cache_write_tokens` 和 cache write cost，不能只假设有缓存收益。
...
- Indirect Prompt Injection paper 已于 2026-07-12 复核 arXiv HTTP、HTML metadata、API v2 metadata 和摘要，可支撑外部内容模糊数据/指令边界、远程注入到被检索数据、应用功能和 API 调用风险的保守表述；它的 2023 real-world examples 不能外推为当前产品表现。OWASP GenAI LLM Top 10 2025 archive 的关键风险项已完成第一轮精读，可支撑 prompt injection、敏感信息泄露、LLM output validation、工具/插件访问控制、excessive agency 和 RAG/vector store 风险的保守表述。旧 OWASP 项目页仍保留 version 1.1 摘要；正文引用当前风险项时优先使用 2025 编号和名称。
- AgentDojo paper 已于 2026-07-12 复核 arXiv v3 metadata、PDF、GitHub repo metadata、README、项目站点和 results 页面，可支撑工具型 Agent 安全评测应覆盖 dynamic tool calling、非可信工具结果、stateful environment、用户目标 / 攻击者目标、injection endpoints、utility/security checks 和 adaptive attacks 的窄边界；论文结果不能外推为当前模型表现、生产 guardrail / detector / HITL 有效性或 tool filtering 充分安全。
- CaMeL / Defeating Prompt Injections by Design 已于 2026-07-12 复核 arXiv v2 metadata、GitHub repo metadata、README 和关键源码入口，可支撑 prompt injection 防护应分离 trusted control/data flow、限制 untrusted data 影响 program flow，并用 capability / security policy 约束工具调用和数据外流的安全架构边界；README 明确代码是 research artifact，可能有 bug 且不 fully secure，因此不能写成受支持生产框架或真实防护效果已验证。
- OWASP Agentic AI Security Resources 的公开资源页、HTTP metadata、WP JSON 摘要和附件列表已在 2026-07-12 复核，可支撑 goal hijacking、tool misuse、identity / privilege abuse、memory poisoning、insecure inter-agent communication、cascading failures、rogue agents、runtime containment、architectural monitoring 和 schema controls 等 agentic-specific 风险边界。`Agentic AI - Threats and Mitigations` 下载端点仍重定向到 no-access，media API 只公开封面 PNG，白皮书全文未精读，因此不能支撑完整缓解清单或具体控制效果。
...
- NIST AI RMF 主页面、AIRC Playbook landing、Playbook JSON/CSV/PDF 下载端点和 Critical Infrastructure concept note 已在 2026-07-12 复核，可支撑生产化章节的 voluntary risk management、trustworthiness lifecycle、Govern / Map / Measure / Manage 和高风险场景治理视角。Playbook JSON/CSV 的结构化条目可作为 production safety / data governance checklist 的字段参考，例如 actor、topic、action、documentation、inventory、incident response、third-party risk、feedback 和 decommissioning；但 Playbook 不是必须完整照做的 checklist，也不证明任何项目合规或控制有效。AI RMF 1.0 与 GenAI profile 的 DOI/PDF 当前均取回失败，NIST 也不是 Agent 专用工程指南。
- Google Responsible AI 页面、旧 URL 跳转链、HTTP metadata 和 Last updated 标记已在 2026-07-12 复核，可支撑生产安全里的保守治理边界：LLM 可能产生 unexpected / offensive / factually incorrect 输出；built-in content filtering 和 safety attribute scoring 只支撑测试 filters 和配置 use-case confidence thresholds；应用仍需要按 use case、users 和 business context 做 security risk assessment、safety testing、必要时配置 filters、收集 user feedback 并 monitor content。该页面不证明任意 safety filter、grounding、监控或 Google 模型的真实效果。
- “Prompt injection 不能只靠 prompt 解决；外部内容应被当作不可信数据，工具权限和写操作审批必须由应用/系统层控制”已升级为可入正文。Indirect Prompt Injection paper 2026-07-12 复核支撑外部检索数据中的恶意 prompt 可影响应用行为和 API 调用的风险边界，但不证明当前 guardrail 或 HITL 效果；OWASP 支撑风险分类，NIST / AIRC 支撑 voluntary risk management、Govern / Map / Measure / Manage 和 suggested-actions 字段化治理入口但不支撑具体防护有效性，Microsoft Prompt Shields 支撑 user prompt attack / document attack 分类、生成前检测接口和误报/漏报边界，OpenAI 工具调用文档支撑应用侧执行和控制边界。
...
- 长期记忆可能提升持续交互体验，但也会引入错误写入、过时、权限、短期/长期上下文混用和隐私风险；MemoryBank / MemGPT / Generative Agents 2026-07-12 arXiv API 复核只支撑持续交互记忆、显式 memory management 和 memory-reflection-planning 组合的研究机制，不证明生产系统默认受益。生产系统不能默认自动写入或默认提升表现，必须把写入守门、跨用户隔离、inspect/edit/delete、失效历史、短期原始上下文保留和敏感 trace 脱敏纳入权限模型。
...
- OpenAI Batch / Flex / Prompt Caching 文档已于 2026-07-12 复核，可支撑“优化手段需要按 workload 选择并记录观测字段”的边界：Batch 应记录 request/file 上限、status、error file、`custom_id` 映射、过期和 output file 删除；Flex 应记录 beta / limited model availability、timeout、`429 Resource Unavailable` 和 fallback；Prompt Caching 应记录 `prompt_cache_key`、explicit breakpoints、`cached_tokens`、`cache_write_tokens`、cache write cost、TTL/retention 和 cache miss 原因。
...
- [AgentDojo: A Dynamic Environment to Evaluate Prompt Injection Attacks and Defenses for LLM Agents](../sources/source-cards/2024-agentdojo-paper.md)
- [Defeating Prompt Injections by Design](../sources/source-cards/2025-camel-prompt-injection-paper.md)

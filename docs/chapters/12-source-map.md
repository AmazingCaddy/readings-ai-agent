...
- 真实 Function Calling / Responses API 实验中，模型能否稳定根据 tool validation error 修正参数？不同框架如何表示 tool execution error？Real Tool Calling harness 已完成 no-key 本地 deterministic validation/retry control；真实 API completed run 仍待做。Semantic Kernel native plugin 已补本地参数处理和应用层审批 wrapper 观察，但还没有真实模型 tool selection、OpenAPI/MCP plugin 或 HITL UI 结果。

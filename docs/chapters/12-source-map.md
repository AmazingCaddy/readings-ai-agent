...
- Agent 架构模式边界已补论文 / 框架资料、Voyager embodied lifelong agent 资料、LangGraph current overview / interrupts / persistence / thinking-in-langgraph / historical examples 和多组标准库对比实验，窄结论“ReAct 是推理/行动交替模式”“Reflexion 需要可校验反馈、范围控制和 trace”“Tree of Thoughts 是搜索式推理路径而非生产编排框架”“Voyager-style automatic curriculum / skill library / environment feedback 是开放式具身 Agent 的研究机制”“复杂 Agent 架构不是默认更可靠，需要用 trace、成本、失败原因、权限和实验结果比较”已可入正文；ReAct / Reflexion / Tree of Thoughts / Voyager 2026-07-12 复核补强来源版本、摘要、项目站点 / README 和论文实验边界，但不证明真实模型或生产任务默认收益；LangGraph 2026-07-12 复核补强 stateful workflow/agent、durable execution、HITL、event streaming interrupts、checkpointer/store、raw state、error routing 和 checkpoint retention 的工程边界；Voyager-style toy environment 已覆盖 curriculum task、技能修正、verified skill library、sandbox rejection 和 stop condition 的最小 trace 形状；仍缺真实模型 / 框架 / Voyager / Minecraft 或其他环境任务中的收益、成本、延迟和错误恢复实验。
...
- Memory / 知识库治理已补标准库写入守门模拟实验、生命周期权限审计和 Real LangGraph Memory Store Validation，窄结论“长期记忆不能默认自动写入或默认提升表现，必须配套写入守门、生命周期权限、跨用户隔离、短期/长期上下文分层和 trace 脱敏”已可入正文；MemoryBank / MemGPT / Generative Agents 2026-07-12 复核补强持续交互记忆、显式 memory management、memory tiers、memory-reflection-planning 的研究机制边界；LangGraph `InMemoryStore` run 支撑 namespace、put/get/search/delete 和应用层历史包装的窄观察，并显示 broad prefix search 可跨 user namespace；Letta/Zep 2026-07-12 复核补强文档化 memory governance 模式，但仍缺真实多会话 Agent / memory framework 的收益、污染、权限、隐私、持久化、删除一致性、成本、延迟和用户查看/编辑/删除 UI 实验。

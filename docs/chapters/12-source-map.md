...
Tool use 连接外部 API、搜索、计算器等工具能力的基础概念已可入正文；Function Calling 本身不执行工具的工程边界也已可入正文。跨框架 tool / function / plugin / retriever / flow 术语对照已完成第一轮，可帮助理解不同框架抽象层级；真实模型参数修正、真实框架默认行为和成本仍需后续实验。
...
公开 benchmark 不能直接代表真实业务 Agent 质量的窄边界已可入正文：AgentBench 和 WebArena 适合学习交互环境、长程任务、functional correctness 和失败分类，OpenAI Evals 适合学习 custom/private eval；业务系统仍需自己的任务集、trace、权限检查和回归集。
...
- Tool Use / Function Calling 已补 Toolformer、OpenAI Function Calling / Responses API、标准库参数校验/重试模拟实验和跨框架术语对照，窄结论“tool use 可以连接外部工具能力”“应用层校验、错误回传、有限重试”和“不同框架术语不能直接互换”已可入正文；仍缺真实 Function Calling / Responses API 实验和真实框架同任务对照。

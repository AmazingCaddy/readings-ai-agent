...
框架选择 rubric smoke test 已经完成，可帮助理解如何把框架比较拆成 required、nice-to-have、avoid、missing required 和 cautions；窄结论“框架应按任务难点和能力边界比较，不能写成某个框架默认最好”已可入正文。Real Framework Same-Task Comparison 已补一个本地同任务 run：同一退款政策检索 + 审批退款任务分别用 OpenAI Agents SDK `FunctionTool` schema / `needs_approval` / direct `ToolContext` invocation、LangGraph `StateGraph` / conditional edge、LlamaIndex `VectorStoreIndex` / retriever source nodes 和 Semantic Kernel plugin metadata / `Kernel.invoke()` 跑通，并明确哪些能力来自框架、哪些来自应用层 policy / side effect / redaction。`openai-agents==0.18.2` 与 `semantic-kernel==1.36.0` 当前需要分两组临时依赖运行；这属于依赖组合边界，不是框架能力排名。它适合学习比较方法，但不证明真实模型、OpenAI Agents SDK `Runner` managed loop、真实 HITL UI、hosted tracing、成本、延迟或生产可靠性。模型 tool selection 和完整生产面横向实验仍需后续试跑。

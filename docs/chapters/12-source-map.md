...
Real Agentic Security Regression Set 实验设计已经把这些来源整理成 case matrix、记录字段、对照组和通过标准；它适合后续迁移到真实模型 / 框架实验，但当前仍是设计，不是运行结果。

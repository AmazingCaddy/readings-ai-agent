...
- RAG / Memory 已补标准库对比实验，可支撑外部知识、thread state、guarded long-term memory 和无安全来源拒答的分层边界；RAG 已补 LlamaIndex source-node harness，Memory 已补 LangGraph `InMemoryStore` harness；Letta/Zep 2026-07-12 复核进一步支撑 memory block editing、version/direct inspection、temporal facts、cross-thread retrieval 和 ingestion delay 等文档化治理模式；仍缺真实 RAG / memory framework / 多会话质量实验。

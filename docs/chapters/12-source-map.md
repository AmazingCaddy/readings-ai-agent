...
先读手册 09，再看 OWASP LLM Top 10、NIST AI RMF、Microsoft Prompt Shields、Anthropic jailbreak / prompt injection mitigation、Google Responsible AI、OpenAI Production / Cost / Latency / Rate Limit 文档、OpenAI Batch / Flex / Prompt Caching 文档、OpenAI Agents SDK guardrails / human-in-the-loop / tracing 文档，以及 Semantic Kernel Plugins 文档。
...
- Security / Production：Indirect Prompt Injection paper、OWASP LLM Top 10、NIST AI RMF、Microsoft Prompt Shields、Anthropic jailbreak mitigation、Google Responsible AI、OpenAI Production / Cost / Latency docs、OpenAI Batch / Flex / Prompt Caching docs、OpenAI Agents SDK、Semantic Kernel。
...
核心 source cards 已经完成多轮链接、元数据和关键段落复核，能支撑章节中的学习方向、术语边界和部分验证结论。主索引已同步这些状态：论文、官方文档、框架文档和安全资料会标注“关键段落已精读 / 部分验证 / 下一步真实实验”。OpenAI Cookbook 已完成 Structured Outputs、File Search RAG、OpenAI Evals、Agents SDK trace/eval、Usage/Cost 和 Rate limits 等具体 recipe 的第一轮复核；OpenAI Evaluation guides 已补 eval-driven development、trace grading、dataset/eval run、LLM-as-judge caveats 和平台退役边界；OpenAI Graders docs 已补 grader 类型、tool-call grading、Python grader 约束和 reward hacking 风险；OpenAI Production / Cost / Latency docs 已补 usage dashboard、rate-limit headers、exponential backoff、token counting、latency/cost optimization 和 project isolation 边界；OpenAI Batch / Flex / Prompt Caching docs 已补异步批处理、低优先级处理和缓存读写观测边界。仍处于候选或只完成链接复核的资料，不能支撑过强的工程断言。
...
- Production / Security 已补 Indirect Prompt Injection paper、OWASP/NIST 风险资料、Microsoft Prompt Shields 检测层资料、Google Responsible AI 生产治理资料、OpenAI Production / Cost / Latency / Rate Limit 文档、OpenAI Batch / Flex / Prompt Caching 文档和标准库 prompt injection / tool permission 模拟实验，可支撑外部内容模糊数据/指令边界、user prompt / document attack 分类、prompt-only 风险、写工具阻断、安全测试/监控、trace 脱敏、token/usage 记录、rate-limit headers、retry、平均/P95 latency、预算阈值、Batch status、Flex fallback、Prompt Caching read/write 和降级策略；仍缺真实模型 / 框架 guardrail / 检测层 / safety filter / HITL approval 实验、跨框架权限对比、审计脱敏策略和真实成本/延迟/吞吐实验。

...
- Security / Production：Indirect Prompt Injection paper、OWASP LLM Top 10、NIST AI RMF、OpenAI Agents SDK、Semantic Kernel。
...
- Production / Security 已补 Indirect Prompt Injection paper、OWASP/NIST 风险资料和标准库 prompt injection / tool permission 模拟实验，可支撑外部内容模糊数据/指令边界、prompt-only 风险、写工具阻断和 trace 脱敏流程；仍缺真实模型 / 框架 guardrail / HITL approval 实验、跨框架权限对比和审计脱敏策略。

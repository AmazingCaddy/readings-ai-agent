...
先读手册 01 和 04，再看 ReAct、Reflexion、Tree of Thoughts、LangGraph 文档和 LangGraph current quickstart。
...
先读手册 08，再看 AgentBench、WebArena、τ-bench、OpenAI Evals、LangSmith 和 Phoenix。
...
- Agent 架构：ReAct、Reflexion、Tree of Thoughts、LangGraph docs、LangGraph examples repo。
...
- Eval / Observability：AgentBench、WebArena、τ-bench、OpenAI Evals repo、LangSmith、Phoenix。
...
- Agent 架构模式边界已补论文 / 框架资料、LangGraph current quickstart / historical examples 和多组标准库对比实验，窄结论“ReAct 是推理/行动交替模式”“Tree of Thoughts 是搜索式推理路径而非生产编排框架”“复杂 Agent 架构不是默认更可靠，需要用 trace、成本、失败原因、权限和实验结果比较”已可入正文；仍缺真实模型 / 框架任务中的收益、成本、延迟和错误恢复实验。
...
- Evaluation / Observability 已补 AgentBench、WebArena、τ-bench、OpenAI Evals、标准库 trace-aware eval 模拟实验和 trace schema audit，窄结论“公开 benchmark 不能直接代表真实业务 Agent 质量”“工具/副作用 Agent 不能只看最终答案”“工具 Agent 评测需要关注用户交互、API tools、状态变化和多次试验一致性”和“trace 字段要按 debug/audit/regression/cost/RAG/privacy 用途设计”已可入正文；仍缺真实 Agent trace、τ³-bench 小样本、LLM-as-judge 误判分析、人工复核、平台字段映射和回归集工程案例。
...
- Cookbook 的具体 recipe 已能支撑实践项目路线，SWE-agent 已能支撑 repo issue / coding agent 进阶练习的接口和安全边界，标准库 smoke harness 已支撑项目验收结构和 trace / 失败分类设计，窄结论“Cookbook 示例不是 API 规范或生产保证”已可入正文；仍需要真实本地试跑来确认依赖、成本、失败样例、SWE-agent / mini-SWE-agent toy repo 行为和初学者阻塞点。

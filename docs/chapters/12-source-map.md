...
先读手册 02 和 03，再看 Responses API、Function Calling docs、Toolformer 和 MRKL。
...
先读手册 06，再看 RAG paper、Self-RAG、LlamaIndex、LangGraph memory、MemGPT、MemoryBank、Generative Agents、Letta 和 Zep。
...
先读手册 08，再看 AgentBench、WebArena、τ-bench、OpenAI Evals、LangSmith、Phoenix、Browser Use 和 Playwright。

阅读重点是任务环境、trajectory、trace、offline/online eval、regression set、datasets、feedback 和错误分类。
公开 benchmark 不能直接代表真实业务 Agent 质量的窄边界已可入正文：AgentBench 和 WebArena 适合学习交互环境、长程任务、functional correctness 和失败分类，τ-bench 适合学习动态用户交互、领域 API tools、policy guidelines、数据库状态评测和多次试验一致性，OpenAI Evals 适合学习 custom/private eval；业务系统仍需自己的任务集、trace、权限检查和回归集。
...
Browser Use / Playwright source card 已完成第一轮复核：它支持浏览器 Agent 的动作层、profile/auth 风险、custom tools 和 trace viewer 边界；真实 browser agent 任务成功率、CAPTCHA/stealth、成本、延迟、合规和生产可靠性仍待实验。

### Production、安全与治理

先读手册 09，再看 OWASP LLM Top 10、NIST AI RMF、Microsoft Prompt Shields、OpenAI Agents SDK guardrails / human-in-the-loop / tracing 文档，以及 Semantic Kernel Plugins 文档。

阅读重点是 prompt injection、权限、工具审批、guardrails 的执行位置、数据边界、审计、人工确认、降级、敏感 trace 控制和风险管理。
Prompt injection / tool permission 标准库模拟实验已经完成，可帮助理解 prompt-only 风险、只读/写工具分离、写工具审批拒绝和 trace 脱敏；Microsoft Prompt Shields 补强 user prompt attack / document attack 分类、生成前检测 API 和误报/漏报边界；真实模型、框架 guardrail、检测层和 HITL approval 仍需后续实验。
...
Browser Use 和 Playwright 适合放在 Browser Agent 进阶实践阶段：它们支持理解网页动作、页面状态、文件上传、表单提交、登录态/profile 和 action trace；但 browser agent 练习必须先用 demo site、测试账号、隔离 profile、人工确认和 trace 脱敏，不应直接操作真实购物、付款、投递或重要账号。
...
- Eval / Observability：AgentBench、WebArena、τ-bench、OpenAI Evals repo、LangSmith、Phoenix、Browser Use / Playwright。
- Security / Production：Indirect Prompt Injection paper、OWASP LLM Top 10、NIST AI RMF、Microsoft Prompt Shields、OpenAI Agents SDK、Semantic Kernel。
- Frameworks / Multi-agent：OpenAI Agents SDK、LangGraph、LlamaIndex、AutoGen、CrewAI、Semantic Kernel、Multiagent Debate。
- Practice：OpenAI Cookbook、MCP servers repo、OpenAI Evals repo、Browser Use / Playwright。
...
- RAG 已补 RAG paper、Self-RAG、LlamaIndex 工程流程资料、LlamaIndex examples repo 和标准库最小 pipeline / citation 模拟实验，窄结论“RAG 的动机包括外部知识访问、知识更新和 provenance”“工程 RAG 是 loading、indexing、storing、querying/retrieval、response synthesis 和 evaluation 等阶段组成的可观察 pipeline，不是单个 prompt 技巧；最小可治理 RAG 需要 chunk metadata、retrieval trace、citation/source 绑定、检索必要性/相关性评估和无证据拒答或 `grounded=false` 标记”已可入正文；LlamaIndex examples 补强了 citation/source node、RAG workflow、BM25 和 hybrid retrieval 的示例证据；仍缺真实 embedding / vector store / LLM synthesis、chunk size/top-k/rerank 对比、citation correctness 和成本/延迟实验。
...
- Evaluation / Observability 已补 AgentBench、WebArena、τ-bench、OpenAI Evals、Browser Use / Playwright、标准库 trace-aware eval 模拟实验和 trace schema audit，窄结论“公开 benchmark 不能直接代表真实业务 Agent 质量”“工具/副作用 Agent 不能只看最终答案”“工具 Agent 评测需要关注用户交互、API tools、状态变化和多次试验一致性”“Browser Agent 需要把网页动作、DOM/page state、profile/auth、权限和 trace 纳入 eval / 审计”和“trace 字段要按 debug/audit/regression/cost/RAG/privacy 用途设计”已可入正文；仍缺真实 Agent trace、真实 Browser Agent 对比实验、τ³-bench 小样本、LLM-as-judge 误判分析、人工复核、平台字段映射和回归集工程案例。
- Production / Security 已补 Indirect Prompt Injection paper、OWASP/NIST 风险资料、Microsoft Prompt Shields 检测层资料和标准库 prompt injection / tool permission 模拟实验，可支撑外部内容模糊数据/指令边界、user prompt / document attack 分类、prompt-only 风险、写工具阻断和 trace 脱敏流程；仍缺真实模型 / 框架 guardrail / 检测层 / HITL approval 实验、跨框架权限对比和审计脱敏策略。
- Framework landscape 已补主要框架文档、框架能力交叉表和标准库框架选择 rubric smoke test，窄结论“框架应按任务难点和能力边界比较，不能写成某个框架默认最好”已可入正文；仍缺真实同一任务框架横向实验。
- Cookbook 的具体 recipe 已能支撑实践项目路线，SWE-agent 已能支撑 repo issue / coding agent 进阶练习的接口和安全边界，Browser Use / Playwright 已能支撑 Browser Agent 的动作层、trace 和 profile/auth 风险边界，标准库 smoke harness 已支撑项目验收结构和 trace / 失败分类设计，窄结论“Cookbook 示例不是 API 规范或生产保证”已可入正文；仍需要真实本地试跑来确认依赖、成本、失败样例、SWE-agent / mini-SWE-agent toy repo 行为、Browser Agent 同任务对比和初学者阻塞点。

...
- Agent 架构模式边界已补论文 / 框架资料、Voyager embodied lifelong agent 资料、LangGraph current overview / interrupts / persistence / thinking-in-langgraph / historical examples 和多组标准库对比实验，窄结论“ReAct 是推理/行动交替模式”“Tree of Thoughts 是搜索式推理路径而非生产编排框架”“Voyager-style automatic curriculum / skill library / environment feedback 是开放式具身 Agent 的研究机制”“复杂 Agent 架构不是默认更可靠，需要用 trace、成本、失败原因、权限和实验结果比较”已可入正文；LangGraph 2026-07-12 复核补强 stateful workflow/agent、durable execution、HITL、event streaming interrupts、checkpointer/store、raw state、error routing 和 checkpoint retention 的工程边界；Voyager-style toy environment 已覆盖 curriculum task、技能修正、verified skill library、sandbox rejection 和 stop condition 的最小 trace 形状；仍缺真实模型 / 框架 / Voyager / Minecraft 或其他环境任务中的收益、成本、延迟和错误恢复实验。

...
多 Agent / Flow 控制标准库实验已经完成，可帮助理解角色协作、Flow 控制、重复读取、冲突和消息开销；真实 AutoGen / CrewAI / LangGraph 对比仍需后续实验。
...
- LLM / Context 已补标准库输出解析 / 上下文治理模拟实验，可支撑“schema validation 不等于语义正确”和“长上下文不能替代来源、时效、权限治理”的最小流程说明；仍缺真实 Responses API / Structured Outputs、refusal、semantic validator、retry loop、长上下文 token/latency/cost 和跨模型稳定性实验。

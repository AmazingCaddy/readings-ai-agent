...
LangGraph current docs 的 interrupts / persistence 页面可帮助理解 `interrupt()`、checkpointer、`thread_id`、`Command(resume=...)`、node restart 和副作用幂等限制；历史 plan-and-execute 和 human-in-the-loop notebooks 只能作为 planner / agent / replan 形态参考，不能证明当前 API、真实任务表现或生产审批安全。
...
- agentic-specific regression set 如何覆盖 goal hijacking、tool misuse、identity / privilege abuse、memory poisoning、insecure inter-agent communication、cascading failures 和 rogue agent / runaway loop 停止条件？

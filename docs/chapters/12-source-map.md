...
NIST AI RMF 适合提供治理框架，而不是工具权限实现方案：2026-07-12 复核确认 AI RMF 主页面仍把框架定位为 voluntary use，并强调把 trustworthiness considerations 纳入 design、development、use 和 evaluation；页面也显示 AI RMF 1.0 is being revised。AIRC Playbook 可作为 Govern / Map / Measure / Manage 的 suggested actions 入口，但它明确不是必须完整执行的 checklist。NIST-AI-600-1 GenAI profile 目前只能引用主页面摘要；DOI/PDF 当前取回失败，全文不能算已精读。

...
Tool use 连接外部 API、搜索、计算器等工具能力的基础概念已可入正文；Function Calling 本身不执行工具的工程边界也已可入正文。真实模型参数修正、跨框架术语和成本仍需后续实验。
...
- Tool Use / Function Calling 已补 Toolformer、OpenAI Function Calling / Responses API 和标准库参数校验/重试模拟实验，窄结论“tool use 可以连接外部工具能力”和“应用层校验、错误回传、有限重试”已可入正文；仍缺真实 Function Calling / Responses API 实验和其他框架术语对照。

...
- Memory / 知识库治理已补标准库写入守门模拟实验、生命周期权限审计和 Real LangGraph Memory Store Validation，窄结论“长期记忆不能默认自动写入或默认提升表现，必须配套写入守门、生命周期权限、跨用户隔离、短期/长期上下文分层和 trace 脱敏”已可入正文；LangGraph `InMemoryStore` run 支撑 namespace、put/get/search/delete 和应用层历史包装的窄观察，并显示 broad prefix search 可跨 user namespace；Letta/Zep 2026-07-12 复核补强文档化 memory governance 模式，但仍缺真实多会话 Agent / memory framework 的收益、污染、权限、隐私、持久化、删除一致性、成本、延迟和用户查看/编辑/删除 UI 实验。

...
- RAG 已补 RAG paper、LlamaIndex 工程流程资料和标准库最小 pipeline / citation 模拟实验，窄结论“RAG 的动机包括外部知识访问、知识更新和 provenance”“工程 RAG 是 loading、indexing、storing、querying/retrieval、response synthesis 和 evaluation 等阶段组成的可观察 pipeline，不是单个 prompt 技巧；最小可治理 RAG 需要 chunk metadata、retrieval trace、citation/source 绑定和无证据拒答或 `grounded=false` 标记”已可入正文；仍缺真实 embedding / vector store / LLM synthesis、chunk size/top-k/rerank 对比、citation correctness 和成本/延迟实验。

...
本手册的最小 tool-calling 模拟实验验证了这个控制循环：fake model 第一轮生成 `unit=kelvin`，应用层拒绝执行并返回 `unit must be one of: celsius, fahrenheit`，第二轮 fake model 改成 `unit=celsius` 后应用层才执行工具。这个实验只能支撑流程设计，不证明真实模型总能稳定修正参数；真实 Responses API harness 已准备并接入统一 runner，当前无 API key 只验证 skip 分支，后续需要配置 API key 后记录实际模型行为。
...
- 本地标准库模拟实验复现了参数校验失败、工具错误回传、第二次工具调用、工具执行和最终响应；它支撑“参数校验和有限重试是应用层控制循环”的工程建议。真实 Responses API tool-calling harness 已准备并接入统一 runner，当前无 API key 只验证 skip 分支，仍不能证明真实模型稳定修正参数。
- 跨框架术语对照已完成第一轮文档交叉验证：OpenAI API 的 `function calling`、OpenAI Agents SDK 的 runtime tools / agent-as-tool、Semantic Kernel 的 plugins/functions、LlamaIndex 的 retriever/query engine、LangGraph 的 state graph、AutoGen/CrewAI 的 multi-agent / Flow 抽象都处在不同层级。正文可以确定写成“术语不能直接互换，应比较执行、状态、权限和 trace 边界”；真实框架默认错误处理、重试、权限和成本仍待同任务实验。

## 待验证问题

- 同一任务在不同框架中如何具体定义 tools、functions、plugins、retrievers 或 agent-as-tool？术语对照已完成第一轮文档验证，仍需真实最小实现记录代码、trace 和错误处理差异。
- 真实模型和真实 Function Calling API 中，工具参数校验失败后能否稳定根据错误修正参数？harness 已准备，当前结果为 `skipped`；仍需实际运行并记录模型、schema、错误格式、成本和延迟。

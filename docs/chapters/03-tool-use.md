...
- “Tool use 可以让模型连接外部 API、搜索、计算器等工具”已升级为可入正文的基础概念：Toolformer 论文摘要支持模型可以学习何时调用工具、传什么参数、如何利用工具结果，OpenAI Function Calling / Responses API 支撑现代 API 通过工具定义、schema、调用请求和结果回传组织工具能力；但 Toolformer 不是现代 API Function Calling 的等价定义。

...
- “Tool use 可以让模型连接外部 API、搜索、计算器等工具”已升级为可入正文的基础概念：Toolformer / MRKL 2026-07-12 arXiv API 复核支持模型可以学习何时调用工具、传什么参数、如何利用工具结果，也支持把 LLM 与外部知识源、离散推理模块组合成系统架构的思想；OpenAI Function Calling / Responses API 支撑现代 API 通过工具定义、schema、调用请求和结果回传组织工具能力；但 Toolformer 和 MRKL 都不是现代 API Function Calling 的等价定义，也不证明真实工具选择、参数修正或模块路由默认可靠。

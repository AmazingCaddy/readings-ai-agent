...
- “ReAct 的核心思想是交替生成推理轨迹和任务行动”已升级为可入正文：ReAct 论文支持 reasoning traces 与 task-specific actions 交替、更新计划、处理异常和连接外部环境；但不支持“ReAct 总是优于 workflow”的泛化说法。
- “Reflection / Reflexion 可以利用任务反馈和文字反思改进后续尝试，但不保证稳定提升”已升级为可入正文的机制边界：Reflexion 论文支持语言反馈、reflective text 和 episodic memory buffer 可用于后续尝试；标准库 Reflection / Retry 实验显示，带证据校验的反思重试可以补齐缺失证据，但未验证反思会让错误重复。因此 reflection 必须绑定可校验反馈、范围控制和 trace，真实效果仍需要结合现代模型、真实任务、成本和记忆污染风险复核。
- “Tree of Thoughts 支持搜索式推理路径，但不等同于生产 Agent 编排框架”已升级为可入正文：ToT 论文支持多个中间 thought、不同推理路径、自我评估、前瞻和回溯；但不应直接等同于生产 Agent 编排。

...
- Reflexion 论文支持“语言反馈和 episodic memory 可用于后续尝试”的研究方向，但其效果边界需要结合现代模型和真实任务复核。
...
- Agent 架构模式边界已完成第一轮验证：ReAct、Reflexion 和 Tree of Thoughts 支持不同研究模式，LangGraph / OpenAI Agents SDK 支持现代工程编排边界；标准库 workflow / hybrid / ReAct-like 对比实验支持“先从可控 workflow 或 hybrid 起步”，Planner / Executor 对比实验支持“计划需要 evidence validation 和反馈重规划”的工程路线，但真实模型和框架实验仍待验证。

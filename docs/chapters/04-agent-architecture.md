...
- Voyager paper / repo 已完成第一轮复核：它支撑 embodied lifelong agent 的 research mechanism，包括 automatic curriculum、executable skill library、environment feedback、execution errors 和 self-verification；但不能证明 Minecraft 以外的通用效果、技能库默认提升质量或开放式 Agent 默认适合生产。
...
- 具身 / 开放式学习 Agent 的真实复现仍待验证：需要记录环境依赖、API 成本、skill library 污染、停止条件、sandbox、trace 和 Minecraft 以外的泛化风险。
...
- Embodied lifelong agent 的 automatic curriculum 和 skill library 能否在 toy environment 或业务任务中稳定复用？

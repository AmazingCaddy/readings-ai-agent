...
- Agent 架构模式边界已完成第一轮验证：ReAct、Reflexion 和 Tree of Thoughts 支持不同研究模式，LangGraph / OpenAI Agents SDK 支持现代工程编排边界；标准库 workflow / hybrid / ReAct-like 对比实验支持“先从可控 workflow 或 hybrid 起步”的工程路线，但真实模型和框架实验仍待验证。

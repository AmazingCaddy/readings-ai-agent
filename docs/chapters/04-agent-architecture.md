...
### Embodied lifelong agent

Embodied agent 指 Agent 不只是处理文本，还要在一个环境里行动、观察反馈并持续学习。Voyager 是这类研究的代表案例：它在 Minecraft 中使用 automatic curriculum 推动探索，用 executable skill library 保存和检索复杂行为，并用 environment feedback、execution errors 和 self-verification 改进程序。

对初学者来说，重点不是复现 Minecraft 结果，而是理解这种 Agent 为什么需要环境接口、可执行动作、长期状态、技能复用、错误反馈、停止条件和 sandbox。Voyager 的性能数字来自特定游戏环境和当时的 GPT-4 设置，不能写成普通业务 Agent 或生产系统的通用结论。

本手册的 Voyager-style toy environment 实验把这个机制缩小成三个任务：收集 wood、制作 pickaxe、挖 stone。没有技能库或保存未验证技能时，流程只能完成 1/3；带治理的 curriculum + verified skill library 能根据环境错误修正技能、拒绝 unsafe action，并在自我验证后保存技能，完成 3/3。这个实验只说明最小记录结构和失败模式，不能替代真实 Voyager 或 Minecraft 复现。
...
Anthropic 的 Building effective agents 把这个递进讲得更细：先从 augmented LLM 开始，再按任务需要使用 prompt chaining、routing、parallelization、orchestrator-workers、evaluator-optimizer，最后才进入更开放的 agent loop。这个分层适合初学者给架构命名，但它仍只是工程指南，不能证明某个 pattern 在真实模型任务中稳定优于其他 pattern。
...
LangGraph current quickstart 进一步把这种结构写成代码：`StateGraph` 保存 state，节点分别负责 LLM 调用和工具执行，conditional edge 根据模型是否产生 tool call 决定继续调用工具还是结束。它能帮助初学者理解“Agent loop”也可以被拆成显式状态、节点和边，而不是一团不可控的 prompt。
...
- “ReAct 的核心思想是交替生成推理轨迹和任务行动”已升级为可入正文：ReAct 2026-07-12 arXiv API 复核支持 reasoning traces 与 task-specific actions 交替、更新计划、处理异常和连接外部环境；但不支持“ReAct 总是优于 workflow”的泛化说法。
- “Reflection / Reflexion 可以利用任务反馈和文字反思改进后续尝试，但不保证稳定提升”已升级为可入正文的机制边界：Reflexion 2026-07-12 arXiv API 复核支持语言反馈、reflective text 和 episodic memory buffer 可用于后续尝试；标准库 Reflection / Retry 实验显示，带证据校验的反思重试可以补齐缺失证据，但未验证反思会让错误重复。因此 reflection 必须绑定可校验反馈、范围控制和 trace，真实效果仍需要结合现代模型、真实任务、成本和记忆污染风险复核。
- “Tree of Thoughts 支持搜索式推理路径，但不等同于生产 Agent 编排框架”已升级为可入正文：ToT 2026-07-12 arXiv API 复核支持多个中间 thought、不同推理路径、自我评估、前瞻和回溯；但不应直接等同于生产 Agent 编排、状态管理、权限控制或持久化 runtime。
...
- LangGraph examples repo 已完成第一轮复核：当前 docs / quickstart 是最新学习入口，仓库 `examples/` 已标注归档。历史 plan-and-execute 和 human-in-the-loop notebooks 可作为架构形态参考，但不能当成最新 API 指南或真实效果证明。
- Voyager paper / repo 已于 2026-07-12 复核 arXiv API、HTTP metadata、项目站点、GitHub repo metadata 和 README：它支撑 embodied lifelong agent 的 research mechanism，包括 automatic curriculum、executable skill library、environment feedback、execution errors 和 self-verification；标准库 Voyager-style toy environment 实验进一步验证了 curriculum task、environment feedback、execution error、self-verification、verified skill library、sandbox rejection 和 stop condition 的最小 trace 形状。但这些证据不能证明 Minecraft 以外的通用效果、真实模型能稳定生成技能、技能库默认提升质量或开放式 Agent 默认适合生产。
- Agent 与 Workflow 边界已升级为可入正文：它们不是互斥概念，而是控制权、状态推进、工具调用顺序和运行时决策方式的连续谱；实际系统常把固定流程和 Agent 能力组合成 workflow-agent hybrid。
- Anthropic Building effective agents 已于 2026-07-12 复核，可支撑 augmented LLM、prompt chaining、routing、parallelization、orchestrator-workers、evaluator-optimizer 和 agent loop 的复杂度递进边界，以及 sandbox、guardrails、human checkpoint 和 stop conditions 的保守提醒；真实模型收益、成本和错误恢复仍需实验。
- Agent 自治程度边界已升级为可入正文：自治程度可以作为分类维度，但不代表能力等级；高自治需要更强的权限、停止条件、trace、eval 和成本控制。真实模型、真实框架、成本、权限和工具错误恢复仍待验证。
- Planner / Executor 边界已升级为可入正文：计划需要可执行，执行结果需要证据校验，失败需要反馈给 planner 并记录重规划 trace；一次性计划可能传播遗漏。
- Agent 架构模式边界已升级为可入正文：复杂 Agent 架构不是默认更可靠，选择 ReAct、Planner / Executor、Reflection、Tree of Thoughts、状态图或多 Agent 前，需要用 trace、成本、失败原因、权限和实验结果比较。真实模型和框架任务中的收益、成本和错误恢复仍待验证。
- 具身 / 开放式学习 Agent 的真实复现仍待验证：toy environment 已覆盖最小技能库治理和 sandbox 失败模式；真实 Voyager / Minecraft 或其他环境仍需要记录环境依赖、API 成本、skill library 污染、停止条件、sandbox、trace 和 Minecraft 以外的泛化风险。
...
- Embodied lifelong agent 的 automatic curriculum 和 skill library 能否在真实模型、真实 Voyager / Minecraft 或业务任务中稳定复用？toy environment 只提供了最小确定性对照组。
- workflow-agent hybrid 相比纯 workflow 或高自治 Agent 的收益，在真实模型和框架任务中如何稳定复现？标准库模拟已覆盖控制流差异，但尚未验证模型不确定性、工具错误恢复、成本和延迟。
...
- [Voyager: An Open-Ended Embodied Agent with Large Language Models](../sources/source-cards/2023-voyager-paper.md)
...
- [Voyager-style Toy Embodied Agent 实验结果](../experiments/voyager-style-toy-agent/results-2026-07-11.md)

...
- AgentBench 和 WebArena 是评估 LLM-as-Agent 能力和 Web Agent 任务复杂度的重要 benchmark 资料，但当前手册只把它们用于解释评测思想和复杂度，不用旧实验数值判断当前模型能力。
- WebArena 的 source card 记录了论文摘要中的端到端任务成功率对比，可用于说明真实 Web 环境评测很难，但不能泛化为所有 Agent 的当前能力结论。
- OpenAI Evals repository 可作为 eval 结构、评测数据和回归测试思路的源码 reference；具体实践建议仍需结合当前维护状态和项目场景。
- 结论证据台账已将“Agent eval 为什么要看 trajectory”列为优先升级结论，目前应保守表述。

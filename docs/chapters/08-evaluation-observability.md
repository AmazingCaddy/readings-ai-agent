...
- Browser Use / Playwright 资料已完成第一轮复核，可支撑浏览器 Agent 的动作层和 trace 边界：浏览器任务可能包含打开页面、点击、输入、填表、上传文件、登录态 profile、custom tools 和 human-in-the-loop；Playwright trace viewer 支持按 action 回放页面状态、log、source、network 和 DOM snapshot。真实 browser agent 成功率、成本、延迟、CAPTCHA/stealth、合规和生产可靠性仍需实验。
...
- 固定 Playwright workflow 和 Browser Use browser agent 在同一个 demo page 上的 action trace、DOM state、权限确认、成本和失败原因有什么差异？

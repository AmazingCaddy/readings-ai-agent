...
需要注意：OpenAI 官方文档已经说明 Evals platform 正在退役，现有内容会在过渡期保留，并计划在 2026-11-30 关闭平台。因此本手册引用 OpenAI Evals 时，重点采用 eval 设计原则、trace grading 和 dataset/eval run 的工程思想，不把旧平台入口写成长期稳定教程。
...
OpenAI Graders 文档还提供了更具体的评分器类型：string check、text similarity、score model grader、Python grader 和 multigrader。它也说明工具调用可以用 `sample.output_tools` 检查 tool name 和 arguments。对初学者来说，这些是设计 eval harness 的参考，不是自动正确性的保证。尤其要注意 reward hacking：系统可能学会迎合 grader 拿高分，但在人类专家看来质量更差。
...
- 本地标准库 trace-aware eval 实验显示，final-answer-only scoring 通过 3/3，而 trace-aware scoring 只通过 1/3；过程评分发现了无审批副作用工具和工具错误未恢复。它支撑“Agent eval 不应只看最终答案”的工程建议。真实模型 trace-aware eval harness 已准备并记录无 API key skipped 结果；当前没有 completed run，仍需要真实 LLM-as-judge、人工复核和平台对照实验。
...
- 真实 Agent trace-aware eval 中，规则评分、LLM-as-judge 和人工评审分别会产生哪些误判？标准库 grader misalignment 实验已给出最小误判结构；真实模型 trace harness 已准备并记录无 API key skipped 结果，仍需实际运行和人工复核样例。
- 模型评审与人工评审如何组合，才能减少误判？
- 不同框架的 observability 能力如何比较？
- AgentBench 和 WebArena 的任务设计对现代业务 Agent 有哪些可迁移经验？
- 固定 Playwright workflow、Browser Use browser agent 和 computer-use-style action loop 在同一个 demo page 上的 action trace、DOM/screenshot state、权限确认、action validation、成本和失败原因有什么差异？标准库 browser action trace audit 已给出字段模板，真实 demo page 对比仍待做。

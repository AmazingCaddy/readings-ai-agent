...
2026-07-12 复核的 OpenAI Evaluation best practices 还强调，eval dataset 不应只覆盖 happy path：它应包含 typical cases、edge cases 和 adversarial cases。对 Agent 来说，edge cases 包括格式变化、长上下文、多次工具调用、循环 handoff、jailbreak attempt 和用户提示与系统提示冲突等情况。LLM-as-judge / model graders 可以降低人工成本，但文档仍要求先和 human labels / human feedback 对齐，再谈自动化规模化、成本或延迟优化。
...
AgentBench、WebArena 和 τ-bench 这类 benchmark 的价值在于让你看到 Agent 评测需要交互环境、长程任务、工具、用户交互、状态变化和失败原因分析。它们不应该被简单理解成“某个分数等于产品可用”。严肃使用这些 benchmark 时，还要记录仓库/任务版本、Docker 或自托管环境、数据和登录态准备、环境 reset、资源消耗、trajectory 文件和失败样例；否则只能把它们当作学习评测思想的资料。
...
- AgentBench 已于 2026-07-12 复核 arXiv API、HTTP metadata、摘要、GitHub README 和 raw README，可支撑“Agent eval 需要交互环境、长期推理、决策、指令遵循和失败原因分析”的保守表述；README 当前已加入 Function Calling 版本，并明确旧 v0.1/v0.2、Docker Compose、Freebase、Redis、Python 3.9、WebShop 约 15-16GB RAM 和 ALFWorld 内存/磁盘泄漏等版本与资源边界，不能把 leaderboard 或旧环境结果写成当前业务可用性判断。
- WebArena 已于 2026-07-12 复核 arXiv API、HTTP metadata、摘要、项目页、GitHub README 和 raw README，可支撑“真实 Web Agent 任务需要端到端环境、工具、外部知识、self-hosted websites、functional correctness、environment reset、auto-login cookies、trajectory HTML 和 long-horizon 评测”的保守表述；README 明确 demo sites 只适合理解内容，reproducible evaluation 需要自建网站环境，论文中的旧实验数值和 README 当前复现路径不能泛化为当前模型能力。

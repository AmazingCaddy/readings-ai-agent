...
smolagents 还适合学习工具信任边界：tool 需要 name、description、inputs 和 output_type，`output_schema` 当前只作 informational use；从 Hub 或 MCP server 加载 tool 可能下载并本地执行代码，`trust_remote_code=True` 只能在信任来源且理解风险时使用。Real smolagents validation 已用 `smolagents==1.26.0` 和 deterministic fake models 跑通 `CodeAgent` Python snippet、本地 tool 调用、`ToolCallingAgent` structured tool calls、`output_schema` 非强制校验和默认 local executor `import os` 阻断表面。这些证据只支持框架定位、执行面和 sandbox/trust boundary；不能引用 README benchmark、文档宣传或本地 fake-model run 来证明它真实更好、更快、更便宜、更安全或更适合生产。
...
当前 LangGraph overview / interrupts / persistence / thinking-in-langgraph 文档还适合用来学习审批和恢复的工程细节：`interrupt()` 可以暂停 graph，event streaming 可以暴露暂停状态，resume 依赖 checkpointer 和 `thread_id`；approval workflow、review/edit state、tool 内中断、多 interrupt 恢复和 conditional-edge validation loop 都有明确参考。需要注意的是，恢复时 node 会从头执行，确认点前代码可能重跑，副作用需要幂等或放在确认之后，内存型 checkpointer 重启会丢失，长会话 checkpoint 还需要清理/保留策略。Thinking-in-LangGraph 还强调 state 应保存 raw data，错误要按 transient、LLM-recoverable、user-fixable、retry-exhausted 和 unexpected 分流；这些是编排设计建议，不是生产安全自动保证。
...
AutoGen 文档已于 2026-07-12 复核入口、AgentChat HTML 和 AgentChat `_sources` Markdown；它提供 AgentChat、Agents、Teams、Selector Group Chat、Swarm、Magentic-One、GraphFlow、memory、logging、serialization 和 tracing 等多 Agent 协调抽象。需要注意：多 Agent 示例容易显得强大，但要评估成本、调试、失败恢复、trace 字段和最终决策责任。
...
CrewAI 文档已于 2026-07-12 复核入口、Introduction、First Flow 和 First Crew Markdown。它把系统拆成 Flows 和 Crews：Flow 管理状态和执行控制，Crew 是 Flow 内协作完成特定任务的 agent team。First Flow 展示 Flow 可以组合普通代码、直接 LLM 调用和 crew-based processing；First Crew 展示 JSON-first agents/tasks/crew settings、`process: sequential`、task `context` 和 `memory: true` 等配置形态。需要注意：当前 source card 将其可信度标为 B，正文应重点用来比较抽象和适用场景，而不是引用 enterprise-ready、production-ready、security-focused 或 cost-efficient 这类营销式结论。

CrewAI First Crew 还明确警告：只应运行可信来源的 JSON crew project，因为 custom tools 和 Python references 会在 crew 加载时执行本地 Python。对初学者来说，这个边界很重要：框架配置不是无害文本，尤其在 multi-agent / tool-rich 项目中，配置、工具和本地代码都要纳入权限和审计。
...
同一个 harness 还用 deterministic local node functions 跑通了 LangGraph 1.2.9 的 `StateGraph`、节点函数和 conditional edges。它说明 state graph 也可以表达 researcher -> reviewer 这类多角色流程，并产出可检查 state trace；但角色行为、缺证据规则和脱敏仍是应用层代码，不等于真实模型多 Agent 协作质量。
...
同样，Real Multi-Agent Framework Validation 提供的是 multi-agent / multi-role runtime surface 对照，不是质量对照。它可以帮助你检查 transcript、task output、state trace、终止条件、条件路由和角色边界是否清楚；不能替代真实模型、多轮冲突合并、token/latency/cost 和人工复核实验。
...
- OpenAI Agents SDK、LangGraph、AutoGen 均已完成关键段落精读；AutoGen 入口、AgentChat HTML 和 AgentChat `_sources` Markdown 已于 2026-07-12 复核，可支撑 SDK runtime、orchestration runtime 和多 Agent 协调抽象的保守表述。
- CrewAI source card 当前可信度为 B，但其 2026-07-12 复核的 Introduction / First Flow / First Crew Markdown 可作为 Flows / Crews、regular code + direct LLM calls + crew-based processing、JSON-first crew 配置和配置执行风险的补充证据；不宜单独支撑关键结论或营销式效果判断。
- LangGraph source card 明确其适合状态图、可控 workflow 和复杂任务编排；2026-07-12 复核的 overview / quickstart / interrupts / persistence / thinking-in-langgraph 文档补强了 StateGraph、Graph API / Functional API tool loop、pause/resume、event streaming interrupt observation、multiple-interrupt resume map、checkpointer/store、`thread_id`、raw state、error routing、conditional-edge validation loop、checkpoint retention 和 side-effect idempotency 边界；examples repo 复核确认 `examples/` 已归档，不能当作最新教程入口。Real LangGraph Interrupt Recovery harness 已完成 `MemorySaver` 最小 run 和 `SqliteSaver` 本地 SQLite 同进程 graph 重建恢复 case、双本地 Python 进程 prepare/resume case 和双本地 Python 进程并发 resume case。LangGraph 是特定框架，不应被写成通用定义，也不证明真实生产审批流程默认安全。
...
- 哪些框架更容易实现权限隔离和人工确认？OpenAI Agents SDK guardrail harness 已补 input/output/tool guardrail 位置边界，2026-07-12 文档复核已补 tool-guardrail 覆盖限制、hosted shell approval 限制、trace 默认敏感数据捕获和 serialized RunState 风险；LangGraph 文档机制已补 interrupt / persistence / checkpointer-store / event streaming / checkpoint retention 边界，Real LangGraph Interrupt Recovery 已完成一个最小真实框架 run、本地 SQLite 同进程恢复、双本地 Python 进程恢复和双本地 Python 进程并发 resume case；Real Framework Same-Task Comparison 已补一个本地同任务 runtime 对照，但审批 policy 多数仍是应用层代码；仍需真实同任务对比 OpenAI Agents SDK、LangGraph、Semantic Kernel、MCP 等工具面的审批状态、参数快照、部署式服务恢复、真实服务并发恢复、幂等执行、checkpoint retention、hosted/MCP/Shell/ApplyPatch 工具覆盖和 trace 脱敏。

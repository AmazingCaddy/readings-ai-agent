...
在工程上，Human-in-the-loop 最好是流程节点，而不是最后加一句“请确认”。LangGraph current docs 的 `interrupt()` / resume 机制给了一个具体参考：graph 可以在 critical action 前暂停，把待确认内容交给人类，再基于保存的 state 恢复；event streaming 还能通过 `stream.interrupted` / `stream.interrupts` 暴露暂停状态。这个例子也提醒我们，编排不是只画流程图，还要处理恢复语义：node 恢复时可能从头执行，确认点前的代码可能重跑，有副作用的动作需要放在确认之后或做成幂等；多个 interrupt 需要按 interrupt ID 配对恢复；表单校验这类循环不宜在单个 node 里 `while True` 反复 interrupt，而应通过 conditional edge 回到节点。
...
AutoGen 和 CrewAI 的文档都说明多 Agent 已经是成熟工程生态的一部分；但这些文档主要说明“可以怎么组织”，不等于证明“默认应该这样组织”。2026-07-12 复核的 AutoGen AgentChat Markdown 明确列出 Agents、Teams、Selector Group Chat、Swarm、Magentic-One、GraphFlow、logging 和 tracing 等入口；CrewAI Introduction / First Flow / First Crew Markdown 则把系统拆成 Flow 和 Crew，并展示 Flow 可以组合普通代码、直接 LLM 调用和 crew-based processing。尤其是 CrewAI 文档明确建议 production-ready application 先从 Flow 开始，再在需要自治协作的步骤中调用 Crew。

Multiagent Debate 论文展示了另一个研究方向：让多个模型实例分别提出答案和推理过程，再经过多轮辩论形成共同答案。2026-07-12 复核的 arXiv API、项目页和 README 支撑这个研究机制，也提示其代码是 preliminary implementation；它说明多视角和互相审查可能有价值，但不等于工程系统里角色越多越好。实际采用前仍要看冲突如何合并、谁负责最终结论、成本是否可接受，以及 trace 是否能审计每个角色的贡献。
...
本手册还补了一个本地真实框架 surface 实验：用 fake model / fake LLM 跑通 AutoGen AgentChat 的 researcher/reviewer `RoundRobinGroupChat` 和 CrewAI 的 sequential `Crew`。两者都能产生可检查的 transcript 或 task output，并暴露缺少 `feedback.md` 的复核结论。这个结果只说明框架可以表达多角色编排和输出 trace；缺证据规则、是否审批、trace 脱敏和最终判断仍是应用层责任。CrewAI First Crew 还提醒，JSON crew project 的 custom tools 和 Python references 会在加载时执行本地 Python；所以 crew 配置也要按执行面管理，不能只当文本配置看待。
...
如果流程需要暂停后恢复，还要明确“用什么持久化”。LangGraph persistence 文档把 checkpointer 用于保存 thread 的 graph state snapshots，把 store 用于跨 thread 的应用数据。对初学者来说，最重要的不是记住这些 API 名字，而是理解：审批恢复依赖可恢复的状态、稳定的 thread 标识和持久化存储；内存型保存器只适合教程或本地实验，进程重启会丢 checkpoint；长会话还要考虑 checkpoint 累积、pruning / retention 和 subgraph checkpoint namespace。
...
- “ReAct 的核心思想是交替生成推理轨迹和任务行动”是本章可入正文的规划机制解释；2026-07-12 arXiv API 复核支撑它能帮助理解边推理、边行动、再根据观察调整下一步，但不证明 ReAct 在真实任务中总优于固定 workflow。
- “Tree of Thoughts 支持搜索式推理路径，但不等同于生产 Agent 编排框架”已升级为可入正文：2026-07-12 arXiv API 复核支撑它适合解释多个中间 thought、不同推理路径、自我评估、前瞻和回溯等搜索式规划思路，但不等同于生产编排框架、状态管理、权限控制或持久化 runtime。
- “Reflection / Reflexion 可以利用任务反馈和文字反思改进后续尝试，但不保证稳定提升”已升级为可入正文的机制边界：2026-07-12 arXiv API 复核支撑 linguistic feedback、reflective text 和 episodic memory buffer 的研究机制；带 verifier 的反馈可帮助补证据，未验证反思会让错误重复。该模式必须绑定可校验反馈、范围控制和 trace；真实收益仍需要结合反馈质量、成本、人工评审和错误记忆风险保守使用。
...
- AutoGen 文档已于 2026-07-12 复核入口、AgentChat HTML 和 AgentChat `_sources` Markdown，可支撑 AgentChat、Agents、Teams、Selector Group Chat、Swarm、Magentic-One、GraphFlow、logging、serialization 和 tracing 等多 Agent 协调抽象。
- CrewAI 文档已于 2026-07-12 复核入口、Introduction、First Flow 和 First Crew Markdown，可支撑 Flows / Crews 的组合：Flow 管理状态和控制执行，Crew 在 Flow 内协作完成特定复杂任务；First Flow 支撑 regular code / direct LLM calls / crew-based processing 的组合模式，First Crew 支撑 JSON-first agents/tasks/crew settings、task context 和本地 Python 执行风险边界。
- Multiagent Debate 已于 2026-07-12 复核 arXiv API、HTTP metadata、摘要、项目页、GitHub repo metadata 和 README，可支撑“多个模型实例多轮提出、辩论并汇总答案”这一研究机制边界；它不能证明工程多 Agent 在真实任务中默认更可靠。
- “多 Agent 是一种编排选择，不是复杂任务的默认升级路径；引入前应明确角色边界、证据分配、冲突处理、review trace 和成本预算”已升级为可入正文。框架文档支撑多 Agent / 多角色编排能力存在，CrewAI 支撑先用 Flow 控制的保守路线，Agent eval 资料支撑需要用 trace、成本和失败原因评估是否值得；标准库多 Agent 对比实验验证了无控制角色协作的重复读取、缺证据和冲突风险；Real Multi-Agent Framework Validation 已用 fake model / fake LLM 跑通 AutoGen AgentChat 和 CrewAI 的 researcher/reviewer 本地 runtime surface，并用 deterministic local node functions 跑通 LangGraph `StateGraph` / conditional edge researcher/reviewer surface。真实模型、多轮冲突合并、token/latency/cost、人工评审负担和复杂多视角任务收益仍待验证。
- LangGraph 文档可作为状态图和编排工程参考，但其抽象属于具体框架。
- LangGraph current docs 的 overview / quickstart / interrupts / persistence / thinking-in-langgraph 页面已于 2026-07-12 复核，可支撑 StateGraph、Graph API / Functional API tool loop、pause/resume、approval workflow、review/edit state、tool 内中断、event streaming interrupt observation、multiple-interrupt resume map、checkpointer/store、`thread_id`、raw state、error routing 和 conditional-edge validation loop 的编排边界；同时也明确 node restart、interrupt 顺序、JSON-serializable payload、in-memory checkpoint loss、checkpoint retention 和 side-effect idempotency 等限制。
- LangGraph examples repo 已于 2026-07-12 复核 README 和 contents API；它可作为 planner/replan、interrupt/resume、multi-agent、RAG、reflection/reflexion 和 tool-calling 等历史形态参考。但 `examples/` 目录已明确归档，不能作为最新用法或生产可靠性的证据，当前学习入口应优先使用 docs overview / quickstart。

## 待验证问题

- Planner / Executor 在真实工程任务中如何定义接口最稳定？标准库实验已验证最小证据校验和重规划 trace，仍需真实模型 / 框架 / repo issue 实验。
- 哪些任务适合多 Agent，哪些任务只需要 workflow？标准库实验显示清晰写作任务用单流程也能完成，AutoGen/CrewAI/LangGraph 本地 deterministic run 只验证了框架编排表面，仍需真实模型 / 框架实验覆盖更复杂多视角任务。
...
- Human-in-the-loop 应该放在流程中哪些位置？LangGraph current docs 已给出 critical actions 前中断、review/edit state、tool 内中断、event streaming 暂停观察和多 interrupt 恢复的机制参考；Real LangGraph Interrupt Recovery completed run 已在 `MemorySaver` 最小 graph 中验证批准执行一次、拒绝不执行、参数 hash 不匹配阻断、重复 resume 不重复执行和 trace 未泄露示例 secret marker，也用 `SqliteSaver` 在本地 SQLite checkpoint 上验证了同进程重建 saver / graph 后恢复、prepare / resume 两个本地 Python 进程之间恢复、两个本地 Python resume 进程同时恢复时共享副作用日志只记录 1 次工具执行。真实业务仍需部署式服务重启、真实服务并发恢复、真实副作用事务、审批 UI、状态表、checkpoint retention 和生产审计验证。

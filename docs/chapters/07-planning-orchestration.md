...
- Human-in-the-loop 应该放在流程中哪些位置？LangGraph current docs 已给出 critical actions 前中断、review/edit state 和 tool 内中断的机制参考；Real LangGraph Interrupt Recovery completed run 已在 `MemorySaver` 最小 graph 中验证批准执行一次、拒绝不执行、参数 hash 不匹配阻断、重复 resume 不重复执行和 trace 未泄露示例 secret marker。真实业务仍需持久化 checkpointer、并发恢复、真实副作用事务、审批 UI、状态表和生产审计验证。

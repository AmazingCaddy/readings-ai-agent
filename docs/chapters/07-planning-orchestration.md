...
- Human-in-the-loop 应该放在流程中哪些位置？LangGraph current docs 已给出 critical actions 前中断、review/edit state 和 tool 内中断的机制参考；Real LangGraph Interrupt Recovery harness 已准备但当前因未安装 LangGraph 只验证 skip 分支，真实业务仍需 completed run 验证审批状态、参数快照、重复恢复、拒绝恢复、幂等执行和 trace 脱敏。

...
本手册还补了一个本地真实框架 surface 实验：用 fake model / fake LLM 跑通 AutoGen AgentChat 的 researcher/reviewer `RoundRobinGroupChat` 和 CrewAI 的 sequential `Crew`。两者都能产生可检查的 transcript 或 task output，并暴露缺少 `feedback.md` 的复核结论。这个结果只说明框架可以表达多角色编排和输出 trace；缺证据规则、是否审批、trace 脱敏和最终判断仍是应用层责任。

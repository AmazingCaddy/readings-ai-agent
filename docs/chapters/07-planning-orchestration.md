...
- Reflexion 支持“语言反馈和反思可用于后续尝试”的研究方向；Reflection / Retry 标准库实验进一步说明，带 verifier 的反馈可帮助补证据，未验证反思会让错误重复。该模式仍需要结合反馈质量、成本和错误记忆风险保守使用。
- Agent 架构模式边界已完成第一轮验证：搜索式推理、反思、状态图和 workflow-agent hybrid 是不同层级的模式，不应被写成复杂任务的默认升级路径。
- Planner / Executor 对比实验已完成标准库模拟：一次性计划可能遗漏关键证据，带校验反馈的重规划可以补齐缺失步骤；仍需真实模型、真实框架和真实任务验证。
...
- “多 Agent 不是默认更好”已升级为部分验证：框架文档支撑多 Agent 能力存在，CrewAI 支撑先用 Flow 控制的保守路线，Agent eval 资料支撑需要用 trace、成本和失败原因评估是否值得；标准库多 Agent 对比实验验证了无控制角色协作的重复读取、缺证据和冲突风险。

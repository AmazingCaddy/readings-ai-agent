...
- Structured Outputs 与 JSON mode 的差异已完成第一轮验证：前者关注 schema adherence，后者主要保证 valid JSON；两者都不等于事实正确性保证。
- “长上下文不能替代上下文治理”已完成第一轮交叉验证：官方文档支持 context window 和 truncation 是工程限制，RAG/Memory 与 Prompt Injection 证据支持检索、状态、权限和评测仍然必要；仍需要本地实验加强。

...
- “LLM 应用输入输出不只是字符串”已升级为可入正文：OpenAI Text Generation 和 Responses API 文档支持输入可以有 role、content type 和 instructions；输出可能包含 output message、tool call、refusal、structured outputs 等多种 item。Text Generation 2026-07-12 复核还确认 SDK `output_text` 只是聚合文本的快捷属性，不能替代结构化 `output` item 解析；生产 prompt 应优先放在代码中管理，reusable prompt objects 正在退役。真实 API refusal/retry、跨模型稳定性、其他供应商字段名和成本仍需验证。
- 结构化输出和工具调用密切相关，但结构化输出不等于工具调用；它也可以用于分类、路由、状态更新和 UI 数据。
- “Structured Outputs / schema validation 提升可解析性和 schema adherence，但不保证事实正确、权限正确或业务正确”是本章可入正文的确定性工程边界。真实 Responses API harness 已准备并接入统一 runner；当前无 API key 时会运行本地 deterministic schema/semantic control，并标记 `real_api_validated=false`。refusal、retry、跨模型稳定性和成本的真实 API completed run 仍待跑，不能提前写成真实模型稳定结论。
...
- 结构化输出在失败重试时的最佳实践有哪些官方 examples？真实 Structured Outputs / JSON mode harness 已准备；当前无 API key 结果为本地 deterministic control，仍需配置 API key 后记录真实 refusal、semantic validator 和 retry 行为。

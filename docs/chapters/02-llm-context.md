...
在本手册的结构化输出模拟实验中，`schema_validated` 策略达到 3/3 schema valid，但仍有 1 个 ticket 语义错误：它没有要求人工 review，并引用了外部 attachment 作为依据。这说明 schema 可以检查字段、类型和 enum，却不能替代业务规则、权限判断和事实校验。真实 Responses API harness 也已准备 free text、JSON mode 和 `json_schema` 对照入口；当前无 API key 时已运行本地 deterministic schema/semantic control，能复现 parse failure、schema-valid 但业务错误、schema-valid 且业务正确三类应用层判断，但还不能证明真实模型的 refusal、retry、成本或稳定性。
...
- OpenAI Responses API 文档是理解现代 OpenAI 应用接口和响应结构的重要官方 reference；2026-07-12 复核确认本章涉及的 `input`、message roles、output items、`include`、conversation / `previous_response_id` state、instructions、structured outputs、refusal、context management、`text.format` / `json_schema` 和 truncation 都是结构化接口边界，具体字段仍需随文档版本复核。
- “LLM 应用输入输出不只是字符串”已升级为可入正文：OpenAI Text Generation 和 Responses API 文档支持输入可以有 role、content type 和 instructions；输出可能包含 output message、tool call、refusal、structured outputs、reasoning/logprobs/search/code/computer 等可通过 `include` 请求的附加数据。Text Generation 2026-07-12 复核还确认 SDK `output_text` 只是聚合文本的快捷属性，不能替代结构化 `output` item 解析；生产 prompt 应优先放在代码中管理，reusable prompt objects 正在退役。真实 API refusal/retry、跨模型稳定性、其他供应商字段名、truncation 行为和成本仍需验证。
- 结构化输出和工具调用密切相关，但结构化输出不等于工具调用；它也可以用于分类、路由、状态更新和 UI 数据。
- “Structured Outputs / schema validation 提升可解析性和 schema adherence，但不保证事实正确、权限正确或业务正确”是本章可入正文的确定性工程边界。2026-07-12 复核补强 supported models、JSON mode explicit instruction / edge cases、refusal、mistakes 和 schema/type divergence 边界；真实 Responses API harness 已准备并接入统一 runner；当前无 API key 时会运行本地 deterministic schema/semantic control，并标记 `real_api_validated=false`。refusal、retry、跨模型稳定性、模型支持差异和成本的真实 API completed run 仍待跑，不能提前写成真实模型稳定结论。
- “长上下文不能替代上下文治理”已升级为可入正文：官方文档支持 context window、context management 和 truncation 是工程限制；Responses API 2026-07-12 复核确认 `truncation` 默认 disabled 时超出上下文会 400，auto 会从对话开头丢弃 items。Google Responsible AI 文档 2026-07-12 复核补强 grounding/factuality、数据质量、长度/结构限制、安全测试和监控边界，并提示若输入或输出超过最大 token limit，Google Agent Platform 的 safety classifiers are not applied；RAG/Memory 与 Prompt Injection 证据支持检索、状态、权限和评测仍然必要；上下文策略对比实验已覆盖 full stuffing、recency-only、lossy summary、keyword RAG 和 governed context 的最小失败模式。真实长上下文 / RAG / 摘要策略的质量、成本、延迟、截断影响、safety classifier 行为和跨模型稳定性仍需实验。

## 待验证问题

- 不同模型和 API 对 system/developer/user/tool 消息的优先级是否有明确官方说明？
- reusable prompt objects 退役后，初学者项目里的 prompt builder、测试、eval 和部署回滚应如何组织成可复用模板？
- 结构化输出在失败重试时的最佳实践有哪些官方 examples？真实 Structured Outputs / JSON mode harness 已准备；当前无 API key 结果为本地 deterministic control，仍需配置 API key 后记录真实 refusal、semantic validator、JSON mode edge cases、retry、token/latency/cost 和 truncation 行为。
...
- [Real Structured Outputs / JSON Mode 对比实验结果](../experiments/real-structured-output-validation/results-2026-07-11.md)

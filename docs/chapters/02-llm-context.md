...
- OpenAI Responses API 文档是理解现代 OpenAI 应用接口和响应结构的重要官方 reference，但具体 API 字段需要按文档精读后再写入正文。
- 结构化输出和工具调用密切相关，但结构化输出不等于工具调用；它也可以用于分类、路由和状态更新。
- “长上下文不能替代上下文治理”目前是工程判断，受到 MemGPT、RAG 和 Memory 资料间接支持，仍需要后续用实验和更多工程资料加强。

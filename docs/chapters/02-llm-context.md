...
本手册的上下文治理模拟实验复现了一个最小失败模式：`naive_long_context` 同时放入旧政策、当前政策和外部附件，结果使用了旧政策并服从外部附件里的恶意指令；`governed_context` 只选择当前可信政策，并把外部附件隔离为数据。这个实验不能证明真实模型在长上下文中一定这样失败，但能说明“窗口更大”不等于“来源、时效和权限自动治理”。
...
- 误区五：只看最终答案就能评估模型调用。Agent 需要看过程，包括工具调用和状态变化。
...
- OpenAI Text Generation 和 Responses API 文档支持“模型输入输出不是简单字符串”的工程边界：输入可以有 role、content type 和 instructions；输出可能包含多种 item。
- 结构化输出和工具调用密切相关，但结构化输出不等于工具调用；它也可以用于分类、路由、状态更新和 UI 数据。
- Structured Outputs 与 JSON mode 的差异已完成第一轮验证和标准库模拟实验：前者关注 schema adherence，后者主要保证 valid JSON；两者都不等于事实正确、权限正确或业务正确。真实 Responses API harness 已准备，但结果待跑，不能提前写成真实模型稳定结论。
- “长上下文不能替代上下文治理”已完成第一轮交叉验证和标准库模拟实验：官方文档支持 context window 和 truncation 是工程限制，RAG/Memory 与 Prompt Injection 证据支持检索、状态、权限和评测仍然必要；仍需要真实长上下文 / RAG / 摘要 / 上下文治理对比实验。
...
- Agent 系统必须记录 trace，否则无法可靠评测和调试。

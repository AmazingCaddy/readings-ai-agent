...
在本手册的结构化输出模拟实验中，`schema_validated` 策略达到 3/3 schema valid，但仍有 1 个 ticket 语义错误：它没有要求人工 review，并引用了外部 attachment 作为依据。这说明 schema 可以检查字段、类型和 enum，却不能替代业务规则、权限判断和事实校验。真实 Responses API harness 也已准备 free text、JSON mode 和 `json_schema` 对照入口；当前无 API key 只验证 skip 分支，还不能证明真实模型的 refusal、retry、成本或稳定性。
...
- “Structured Outputs / schema validation 提升可解析性和 schema adherence，但不保证事实正确、权限正确或业务正确”是本章可入正文的确定性工程边界。真实 Responses API harness 已准备并接入统一 runner，当前无 API key 只验证 skip 分支且已有 skipped 结果页；refusal、retry、跨模型稳定性和成本的 completed run 仍待跑，不能提前写成真实模型稳定结论。
...
- 结构化输出在失败重试时的最佳实践有哪些官方 examples？真实 Structured Outputs / JSON mode harness 已准备，当前结果为 `skipped`；仍需配置 API key 后记录 refusal、semantic validator 和 retry 行为。

...
LlamaIndex examples repo 也提供了代码层参考：2026-07-12 复核确认当前正确路径是 `docs/examples/...`，其中 `CitationQueryEngine` 示例会展示 `response.source_nodes`，RAG workflow 示例把流程拆成 indexing、retrieval、rerank 和 synthesis，BM25 retriever 示例覆盖 nodes、docstore、metadata filter 和 hybrid retrieval。它们适合初学者学习真实框架的结构，但不能证明 citation 一定正确、某个 retriever 最好，或示例可以直接上生产。
...
Letta 文档中的 stateful agent persistence、memory blocks、agent/tool/API editing、`/remember`、`/doctor`、git-backed memory、version history 和 direct inspection/editing，以及 Zep 文档中的 temporal graph、fact invalidation、valid/invalid dates、cross-thread User Graph retrieval 和 ingestion delay，都说明了同一个工程事实：长期记忆需要能被检查、分层、整理、纠错和标记时效，而不是只靠模型自动“记住”。这些资料是工程模式参考，不等于证明长期记忆总能提升任务表现、默认安全、默认低延迟或默认适合所有任务。

Mem0 文档和 README 进一步提供了一个偏 API lifecycle 的参考面：长期记忆系统通常需要 `add`、`search`、`get`、`update`、`delete`、`delete_all` 这类操作，并用 `user_id`、`agent_id`、`app_id` 或 `run_id` 之类的实体范围约束查询和写入。它还把 expiration、history、export、events、feedback、webhooks 和 organization/project access control 放进文档入口。对初学者来说，这些接口名比“让 Agent 记住用户”更具体：你至少要问清楚谁拥有这条记忆、何时过期、如何查看、如何导出、如何删除、删除后是否还会被召回，以及这些行为是否真的在你的环境里跑过。Mem0 的 benchmark 和 production-ready claim 仍不能直接写成通用结论，尤其不能把 managed platform claim 外推到 OSS 默认表现。

Zep 的 retrieving-context 文档还有一个对初学者很重要的提醒：Context Block 可能来自同一用户的任意 thread，而且 ingestion 可能需要几分钟，所以实际调用模型时仍建议携带最近 4-6 条 thread messages。换句话说，长期记忆不是当前会话短期状态的替代品；RAG、short-term memory 和 long-term memory 应该分层组合，而不是互相混用。
...
- “RAG 的动机包括外部知识访问、知识更新和 provenance”已升级为可入正文：RAG 论文 2026-07-12 arXiv API 复核支持外部检索可以帮助知识密集型生成任务，并明确提到 provenance 和 world knowledge 更新问题；真实 RAG stack 的质量、citation correctness、成本和延迟仍需实验验证。
...
- “工程 RAG 是 loading、indexing、storing、querying/retrieval、response synthesis 和 evaluation 等阶段组成的可观察 pipeline，不是单个 prompt 技巧”已升级为可入正文：RAG / Self-RAG 2026-07-12 arXiv API 复核支撑外部检索、provenance、知识更新、固定 top-k 盲检索风险和 citation accuracy 评估边界；LlamaIndex 文档支持现代工程 RAG 的五阶段流程，Documents / Nodes、Indexes、Retrievers 和 Query Engines 可作为初学者理解工程组件的参考。
- Self-RAG 论文摘要已完成 2026-07-12 复核，可支撑“不要盲目固定检索；需要评估 retrieval necessity、passage relevance、answer faithfulness 和 citation accuracy”的进阶 RAG 边界。它不能证明当前工程默认应使用 Self-RAG，也不能替代真实 embedding / vector store / rerank / LLM synthesis 实验。
...
- LlamaIndex examples repo 已于 2026-07-12 复核：正确 evidence path 是 `docs/examples/...`，`docs/docs/examples/index.md` raw 路径返回 404，不能继续作为证据；`CitationQueryEngine` / `response.source_nodes`、RAG workflow with reranking、BM25 / hybrid retrieval 等示例可作为工程结构 reference；这些示例不能证明 citation correctness、answer faithfulness、最佳 chunk/top-k/rerank 策略或生产可靠性。
...
- LangGraph memory 文档已于 2026-07-12 复核：它按 recall scope 区分 short-term/thread-scoped memory 和 long-term/cross-session memory，可作为短期/长期记忆工程边界的参考。Letta 文档进一步把 stateful agent 拆成 persisted state、memory blocks、messages、tools 和 conversations；Zep 文档进一步说明 Context Block 可跨同一 user 的 threads 检索，且 ingestion 延迟意味着最近 thread messages 仍要作为 raw short-term context 提供；Mem0 文档进一步补充 CRUD、entity scope、expiration/history/export/events 等 lifecycle API surface。这些资料共同支撑“长期记忆不是完整聊天历史，也不是当前 thread state 替代品”的窄边界。
- LangGraph memory 文档强调 long-term memory 没有 one-size-fits-all solution，写入方式有 hot path 和 background 两类权衡；semantic memory 的 profile / collection 管理分别有 profile 更新 error-prone、collection delete/update/search 复杂度和模型 over-insert / over-update 风险；`InMemoryStore` 示例也明确生产使用应换成 DB-backed store。该条只支撑文档化设计风险和 store caveat，不证明真实模型会正确管理记忆，也不证明生产持久化、删除一致性、成本或可靠性。
...
- MemGPT、MemoryBank、Generative Agents 已于 2026-07-12 完成 arXiv API 复核，可支撑长期记忆、显式 memory management、memory tiers、memory-reflection-planning 组合和持续交互研究方向，但不能泛化为“加长期记忆总是更好”。
- 长期记忆治理与风险边界已升级为可入正文：长期记忆可能适合持续交互、个性化和跨会话任务，但不能默认自动写入或默认提升表现；系统必须配套写入守门、冲突/失效处理、用户可检查/可编辑/可删除、跨用户权限隔离和敏感 trace 脱敏。MemoryBank / MemGPT / Generative Agents 2026-07-12 复核补强研究机制边界，Letta/Zep 2026-07-12 复核补强 persisted memory/state、memory block editing、version/direct inspection、temporal facts、cross-thread retrieval 和 ingestion delay 等工程模式；但真实多会话 Agent / memory framework 的收益、污染率、权限 UI、删除一致性、成本、延迟和用户控制体验仍需实验。
...
- chunk size、top-k 和 rerank/filter 如何影响 citation correctness、answer faithfulness、latency 和 token cost？本地 strategy audit 已验证这些字段应进入 eval 记录，但真实 embedding / vector store / File Search / reranker 表现仍待验证。
...
- [Mem0 Documentation and Paper](../sources/source-cards/2026-mem0-docs.md)

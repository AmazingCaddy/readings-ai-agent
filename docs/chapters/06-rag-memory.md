...
短期记忆通常服务于当前任务或当前会话。例如：当前步骤、刚刚调用过的工具、用户这轮对话里的约束。

短期记忆的生命周期短，重点是帮助 Agent 保持任务连续性。

### 长期记忆

长期记忆跨会话存在。例如：用户长期偏好、项目约定、历史决策、常见错误、业务实体信息。
...
- RAG 论文支持“外部检索可以帮助知识密集型生成任务”这一基础动机，但现代工程 RAG 需要结合框架文档理解。
- MemGPT、MemoryBank、Generative Agents 支持长期记忆和记忆管理的研究方向，但不能泛化为“加长期记忆总是更好”。
- LangGraph memory 文档可作为短期/长期记忆工程边界的参考，但具体抽象属于框架实现。

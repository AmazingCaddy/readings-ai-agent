...
- Real LlamaIndex RAG Source-Node Validation 已完成本地 `llama-index-core` / `VectorStoreIndex` / retriever run：3 个 case 全部通过，untrusted malicious refund note 被检索到但没有进入 citations，无证据 browser/captcha 问题返回 `grounded=false`，trace 未记录示例 secret。它支撑真实框架 retrieval/source-node metadata 的最小观察，但不支撑 LLM synthesis、citation faithfulness、真实 embedding / vector DB / reranker / File Search、成本或延迟结论。
...
- 真实 embedding / vector store / OpenAI File Search / LLM synthesis 下，最小 RAG baseline 应该包含哪些指标？当前 Real LlamaIndex RAG Source-Node Validation 只覆盖本地 `VectorStoreIndex` / retriever / metadata flow，真实 LLM citation harness 已准备并记录无 API key skipped 结果，仍需真实 completed run 并扩展到 embedding / vector store / File Search / rerank。

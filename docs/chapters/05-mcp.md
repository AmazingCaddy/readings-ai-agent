...
本手册的 MCP 最小 trace 模拟实验记录了 `actor`、`method`、`decision`、关键参数摘要和时间戳。实验中，host 批准了只读工具、拒绝了模拟写工具，并把恶意 resource 标记为 prompt injection 候选。这个实验只能说明 trace 字段设计自洽，不能替代真实 MCP SDK 或 host 测试。
...
- “MCP 不是 Agent 框架本身”和“MCP 不自动完成安全边界”已完成第一轮官方文档交叉验证和最小模拟 trace；仍建议补真实 MCP SDK / host 的 trace、权限确认和 URL mode / OAuth 实验。

## 待验证问题

- 如何用真实 MCP SDK / host 复现一次 `tools/list`、`tools/call`、`resources/list`、`resources/read` 和 trace？
- 不同 host 对 tool confirmation、roots、elicitation、sampling 和 audit log 的实现差异有多大？
- 如何把当前标准库模拟中的恶意 resource/prompt 扩展到真实 host，验证 host 是否会把外部内容当成不可信上下文？

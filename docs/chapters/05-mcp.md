...
第一层是 MCP 官方 specification：它定义 host、client、server、tools、resources、prompts、authorization、roots、elicitation、sampling 和 experimental tasks 等协议边界。这里的 tasks 是长耗时请求的状态/结果检索 utility，不等于完整 Agent 任务编排框架。

第二层是具体产品如何接入 MCP。比如 Anthropic 的 MCP connector 文档已于 2026-07-12 复核：它允许开发者通过 Messages API 连接 remote MCP servers，并把这些 server 的 tools 作为可调用工具暴露给 Claude。这个 connector 支持 `mcp_servers`、`mcp_toolset`、OAuth bearer token、allowlist、denylist 和 per-tool configuration；文档还说明 Message Batches API 可以包含 `mcp_servers`，但这只是接口/计费边界，不代表真实批处理效果已验证。

这个能力很有用，但边界也要看清楚：Anthropic 文档说明 MCP connector 当前只支持 MCP tool calls，不等于支持全部 MCP capabilities；也不适用于 ZDR，MCP server 交换的数据、工具定义和执行结果按 Anthropic 标准数据保留政策处理。Local STDIO servers 也不能直接连接到这个 connector；如果需要 local servers、MCP prompts、MCP resources 或更细控制，应使用 SDK helper 或自建 MCP client。

Anthropic 还提供 MCP tunnels 文档，用来把私有网络内的 MCP servers 连接给 Claude，而不需要打开 inbound firewall ports 或把服务暴露到公网。Tunnels 使用 outbound-only connection、proxy、inner TLS 和 shared responsibility model。但 tunnels 仍处于 research preview，按文档是 as-is 且没有 uptime、support 或 continuity commitment；上游 MCP server 仍需要 OAuth、最小 scope、网络限制、日志监控、凭据轮换、breach teardown 和审计。
...
2026-07-12 复核的 MCP servers repo README 仍把仓库定位为 MCP steering group 维护的少量 reference servers，并明确这些 server 是教育示例、不是 production-ready solutions。当前 `src` 目录包含 Everything、Fetch、Filesystem、Git、Memory、Sequential Thinking 和 Time；其中 Everything 是用于测试 MCP protocol features 的 server，Filesystem 展示 allowed directories、Roots 和 read-only / destructive 等 ToolAnnotations，Git 则包含 `git_add`、`git_commit`、`git_reset`、`git_checkout` 等会改变仓库状态的工具。因此这些示例适合学习 server 形态和权限提示，不适合直接证明真实 host 审批、sandbox、rollback 或生产安全。
...
- MCP tasks utility 已于 2026-07-12 复核，支撑 long-running request 的 task state、polling、deferred result retrieval、cancellation 和 authorization-context binding 边界；它仍是 experimental，不能证明真实 host / server 的任务可靠性或通用工作流编排能力。
- Anthropic MCP connector / tunnels 文档已于 2026-07-12 复核，支撑具体产品集成边界：Messages API 可连接 remote MCP tools，并支持 allowlist、denylist、OAuth bearer token、per-tool configuration 和 Message Batches API 中的 MCP tool calls；connector 当前只支持 MCP tool calls，不支持全部 MCP capabilities，且不适用于 ZDR；MCP tunnels 可减少私有网络 MCP server 的公网暴露需求，但仍是 research preview，并需要 OAuth、最小 scope、allowed IPs、凭据保护、日志、breach teardown 和 shared responsibility。
...
- MCP servers repo 已于 2026-07-12 复核，支撑 reference server / educational example / client configuration 的窄边界；Filesystem README 可作为 allowed directories、Roots 和 ToolAnnotations 的示例，Git README 可作为写工具权限风险示例，但它不证明真实 host UI、审批、sandbox、rollback 或生产安全。
- “MCP 是连接工具和上下文能力的协议，不是 Agent 框架本身”是本章可入正文的职责边界；MCP 可以暴露 tools、resources 和 prompts，但不会替代 Agent 的规划、状态管理、权限策略、评测和生产治理。
- “MCP 是协议层，不是完整 Agent 框架”是可入正文的职责边界；“MCP 不自动完成安全边界”也已升级为可入正文：authorization 是 optional，roots 不等于 sandbox，token passthrough 被禁止，高风险 tools/resources/prompts 仍需要最小权限、用户确认、输入输出校验、audience 校验、sandbox/隔离、审计和 trace 脱敏。2025-11-25 tasks 只可写成 experimental long-running request utility。本地 JSON-RPC 和官方 Python SDK harness 已覆盖最小进程 / SDK 流程；真实 host UI、权限确认、URL mode / OAuth、sampling、tasks、tunnel 和恶意 resource/prompt 呈现仍需实验。

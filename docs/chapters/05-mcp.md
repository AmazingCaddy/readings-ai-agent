...
- “MCP 是协议层，不是完整 Agent 框架”是可入正文的职责边界；“MCP 不自动完成安全边界”已完成第一轮官方文档交叉验证、最小模拟 trace 和本地 stdio JSON-RPC harness，仍建议补真实 MCP SDK / host 的 trace、权限确认和 URL mode / OAuth 实验。

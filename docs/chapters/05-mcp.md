...
- “MCP 不是 Agent 框架本身”和“MCP 不自动完成安全边界”已完成第一轮官方文档交叉验证，但仍建议补一个最小 MCP trace / 权限实验。

## 待验证问题

- 如何用只读 server 复现一次 `tools/list`、`tools/call`、`resources/list`、`resources/read` 和 trace？
- 不同 host 对 tool confirmation、roots、elicitation、sampling 和 audit log 的实现差异有多大？
- 如何设计一个最小恶意 resource/prompt，用来验证 host 是否会把外部内容当成不可信上下文？

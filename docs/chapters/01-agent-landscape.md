...
- OpenAI Agents SDK、LangGraph、AutoGen、Semantic Kernel、CrewAI 等资料支持“Agent 框架生态存在多种抽象”，但不应把某一个框架定义当成通用定义。
- 当前“Agent vs Workflow”的边界主要来自术语整理和框架文档对照，仍需要进一步精读官方文档后升级证据状态。

## 待验证问题

- 是否有官方文档对 Agent 和 Workflow 给出直接对比？
- 哪些任务类型有公开案例证明 Agent 优于固定 workflow？
- 初学者是否应该先学 workflow-agent hybrid，而不是直接学 autonomous agent？

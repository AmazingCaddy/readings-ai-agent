...
OpenAI 的入门指南也采用类似边界：Agent 不是所有接入 LLM 的应用，而是让 LLM 管理 workflow execution、选择工具并在失败时停止或交还控制权的系统。这个说法适合初学者理解方向，但不能替代最新 API 文档或真实效果实验。

Anthropic 的 Building effective agents 也把 workflow 和 agent 放在同一个 agentic systems 范围内，但给出更直接的控制权区别：workflow 是 LLM 和工具沿预定义代码路径运行，agent 则让 LLM 动态决定流程和工具使用。它还提醒先找最简单可行方案，必要时再增加复杂度；这支持本章“先判断是否真的需要 Agent”的路线，但不证明任何框架或模型在真实任务中默认更好。
...
判断是否值得做 Agent 时，可以先看任务是否真的有复杂决策、难维护规则或大量非结构化数据。如果这些条件不明显，固定 workflow、规则系统或普通 RAG 问答可能更容易测试和维护。
...
- OpenAI Practical Guide to Building Agents PDF 支持初学者版定义、适用场景判断和“先单 Agent、再按需要多 Agent”的保守路线；该指南不证明真实 API/SDK 行为、模型效果、guardrail 效果或成本延迟。
- Anthropic Building effective agents 支持 workflows vs agents 的控制权区分、简单优先、latency/cost tradeoff 和 framework abstraction caveat；它是工程经验文，不证明真实模型或框架效果。
...
- “Agent 和 Workflow 是控制权与编排方式的连续谱，不是互斥阵营”已升级为可入正文；“自治程度可以作为分类维度，但不是能力等级”也已升级为可入正文。Agent/Workflow evidence 支持控制权连续谱，Prompt Injection evidence 支持 unchecked autonomy 的风险边界，标准库 workflow / hybrid / ReAct-like 对比实验支持灵活性与工具调用成本的权衡。真实模型、真实框架、成本、权限和工具错误恢复仍待验证。
